...
void FileSelect_ConfigModeDraw(GameState* thisx) {
    ...
    Gfx_SetupFrame(this->state.gfxCtx, false, 0, 0, 0, 0);
    Skybox_Draw(&this->skyboxCtx, this->state.gfxCtx, NULL, SKYBOX_NORMAL_SKY, this->envCtx.skyboxBlend, eyeX, eyeY, eyeZ);
...
void FileSelect_Main(GameState* thisx) {
    ...
    if ((this->configMode <= CM_NAME_ENTRY_TO_MAIN) || (this->configMode >= CM_UNUSED_DELAY)) {
        ...
        gDPLoadTextureBlock(POLY_OPA_DISP++, controlsTextures[gSaveContext.language], G_IM_FMT_IA, G_IM_SIZ_8b, 144, 16,
                            ...
                            G_TX_NOLOD, G_TX_NOLOD);
        gSPTextureRectangle(POLY_OPA_DISP++, X_MULTIPLY(HIRES_MULTIPLY(90 + WS_SHIFT_HALF)) << 2, HIRES_MULTIPLY(204 << 2), X_MULTIPLY(HIRES_MULTIPLY(234 + WS_SHIFT_HALF)) << 2, HIRES_MULTIPLY(220 << 2), G_TX_RENDERTILE, 0, 0, HIRES_DIVIDE(1 << 10) / SCALE_X,
                            HIRES_DIVIDE(1 << 10));
    }
