...
void FileSelect_UpdateMainMenu(GameState* thisx) {
    ...
    if ( (CHECK_BTN_ANY(input->press.button, BTN_Z | BTN_DUP) && !CHECK_BTN_ALL(input->cur.button, BTN_L | BTN_R) && EXTRA_SAVE_SLOTS) || (!EXTRA_SAVE_SLOTS && gSaveContext.extraSaveFiles) ) {
