...
static const char* sQuestMessages[4][QUEST_MAX+1][2] = {
    {
        ...
        { "Renne durch alle Dungeons",          "und stelle die Bestzeit auf"  },
        { "Renne durch alle Dungeons",          "und stelle die Bestzeit auf"  },
        { "Behaupte dich gegen alle Bosse",     "und stelle eine Bestzeit auf" },
...
void FileSelect_DrawNameEntry(GameState* thisx) {
    ...
    if (this->selectingQuestMode) {
        ...
        gSPTextureRectangle(POLY_OPA_DISP++, X_HIRES_MULTIPLY(x + WS_SHIFT_HALF) << 2, HIRES_MULTIPLY(y << 2), X_HIRES_MULTIPLY(x + WS_SHIFT_HALF + 32) << 2, HIRES_MULTIPLY((y + 32) << 2), G_TX_RENDERTILE, 0, 0, X_HIRES_DIVIDE(1 << 10), HIRES_DIVIDE(1 << 10));
...
#endif
        sramCtx->readBuff[SRAM_HEADER_GLOBAL_SETTINGS] = gSaveContext.globalSettings;
        PRINTF("ＳＡＶＥ");
...
static OptionsMenuTextureInfo sOptionsMenuHeaders[] = {
    ...
    { { gFileSelCheckBrightnessENGTex, gFileSelCheckBrightnessGERTex, gFileSelCheckBrightnessFRATex },
      { 128, 128, 128 },
      16 },
    { { gFileSelMoreOptionsENGTex, gFileSelMoreOptionsGERTex, gFileSelMoreOptionsFRATex }, { 128, 128, 128 }, 16 },
};
