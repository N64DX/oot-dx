...
void ConsoleLogo_Draw(ConsoleLogoState* this) {
    ...
    for (idx = 0, y = 94; idx < 16; idx++, y += 2) {
        ...
        gSPTextureRectangle(POLY_OPA_DISP++, HIRES_MULTIPLY((WS_SHIFT_HALF + 97) << 2) * SCALE_X, HIRES_MULTIPLY(y << 2), HIRES_MULTIPLY((WS_SHIFT_HALF + 289) << 2) * SCALE_X, HIRES_MULTIPLY((y + 2) << 2), G_TX_RENDERTILE, 0, 0, HIRES_DIVIDE(1 << 10) / SCALE_X,
                            HIRES_DIVIDE(1 << 10));
    }
