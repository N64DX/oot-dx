...
void ConsoleLogo_Draw(ConsoleLogoState* this) {
    ...
    for (idx = 0, y = 94; idx < 16; idx++, y += 2) {
        ...
        gSPTextureRectangle(POLY_OPA_DISP++, X_MULTIPLY(HIRES_MULTIPLY(WS_SHIFT_HALF + 97)) << 2, HIRES_MULTIPLY(y << 2), X_MULTIPLY(HIRES_MULTIPLY(WS_SHIFT_HALF + 289)) << 2, HIRES_MULTIPLY((y + 2) << 2), G_TX_RENDERTILE, 0, 0, HIRES_DIVIDE(1 << 10) / SCALE_X,
                            HIRES_DIVIDE(1 << 10));
    }
...
void ConsoleLogo_DrawQuestImageRGBA32(GraphicsContext* gfxCtx, s16 centerX, s16 centerY, u8* source, u32 width, u32 height) {
    ...
    for (i=0; i<textureCount; i++) {
        ...
        gSPTextureRectangle(POLY_OPA_DISP++, X_MULTIPLY(HIRES_MULTIPLY(rectLeft + WS_SHIFT_HALF)) << 2, HIRES_MULTIPLY(rectTop << 2), X_MULTIPLY(HIRES_MULTIPLY(rectLeft + WS_SHIFT_HALF + (s32)width)) << 2, HIRES_MULTIPLY((rectTop + textureHeight) << 2), G_TX_RENDERTILE, 0, 0, HIRES_DIVIDE(1 << 10) / SCALE_X, HIRES_DIVIDE(1 << 10));
