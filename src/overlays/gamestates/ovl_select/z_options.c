...
void FileOptions_SetResumeLastArea() {
    TOGGLE_RESUME_LAST_AREA;
    Audio_PlaySfxGeneral(NA_SE_IT_SWORD_IMPACT, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
}

void FileOptions_SetCensorFireTemple() {
    TOGGLE_CENSOR_FIRE_TEMPLE;
    Audio_PlaySfxGeneral(NA_SE_IT_SWORD_IMPACT, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
}
...
void FileOptions_UpdateMenu(FileOptionsState* this) {
    ...
    if (this->verticalInputAccumulator == 0) {
        ...
        if (CHECK_BTN_ANY(input->press.button, BTN_B | BTN_START)) {
            gSaveContext.save.linkAge = gSaveContext.save.linkAgeBackup;
            if (!GET_EVENTCHKINF(EVENTCHKINF_SAVED_AFTER_INTRO) && gSaveContext.save.entranceIndex == ENTR_LINKS_HOUSE_0 && gSaveContext.save.linkAge == LINK_AGE_CHILD && gSaveContext.save.dayTime == CLOCK_TIME(10, 0))
...
void FileOptions_PrintMenu(FileOptionsState* this, GfxPrint* printer) {
    ...
    GfxPrint_Printf(printer, "File Options");
...
void FileOptions_Init(GameState* thisx) {
...

    {
        ...
        this->staticSegment = GAME_STATE_ALLOC(&this->state, size, "../z_options.c", 1114);
        DMA_REQUEST_SYNC(this->staticSegment, (uintptr_t)_z_select_staticSegmentRomStart, size, "../z_options.c", 1115);
    }
