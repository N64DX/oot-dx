...
void EnMkk_Destroy(Actor* thisx, struct PlayState* play);
void EnMkk_Update(Actor* thisx, struct PlayState* play);
void EnMkk_Draw(Actor* thisx, struct PlayState* play);

void func_80A4E100(EnMkk* this, struct PlayState* play);
void func_80A4E1F0(EnMkk* this, struct PlayState* play);
void func_80A4E2E8(EnMkk* this, struct PlayState* play);
void func_80A4E60C(EnMkk* this, struct PlayState* play);
void func_80A4E72C(EnMkk* this, struct PlayState* play);
void func_80A4EE48(EnMkk* this, struct PlayState* play);
void func_80A4EF74(EnMkk* this, struct PlayState* play);
...
void func_80A4F16C(Actor* thisx, struct PlayState* play);
void func_80A4F4C8(Actor* thisx, struct PlayState* play);
void func_80A4E67C(EnMkk* this);
void func_80A4E84C(EnMkk* this);
void func_80A4EBBC(EnMkk* this, struct PlayState* play);
...
static CollisionCheckInfoInit sColChkInfoInit = { 4, 15, 30, 10 };
...
static InitChainEntry sInitChain[] = {
    ...
    ICHAIN_F32(velocity, -5, ICHAIN_CONTINUE),
...
void EnMkk_Init(Actor* thisx, PlayState* play) {
    ...
    thisx->colChkInfo.health = Actor_EnemyHealthMultiply(thisx->colChkInfo.health, MONSTER_HP);
    paramsFF00 = ENMKK_GET_FF00(&this->actor);
    params2 = ENMKK_GET_2(&this->actor);
    this->actor.params &= 1;

    if (this->actor.params == 1)
        this->actor.naviEnemyId = NAVI_ENEMY_WHITE_BOE;
    else this->actor.naviEnemyId = NAVI_ENEMY_BLACK_BOE;
...
void func_80A4E22C(EnMkk* this, PlayState* play) {
    ...
    func_8002829C(play, &pos, &gZeroVec3f, &D_80A4F788, &D_80A4F780, &D_80A4F784, 350, 20);
}
...
void func_80A4E2E8(EnMkk* this, PlayState* play) {
    ...
    this->unk_14E--;
    if ((this->actor.params == 1) && (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) && (this->actor.speed > 2.5f) && ((play->gameplayFrames % 3) == 0))
        func_80A4E22C(this, play);
    if (this->unk_14E > 0) {
        Math_StepToF(&this->actor.speed, 5.0f, 0.7f);
        sp20 = false;
    } else sp20 = Math_StepToF(&this->actor.speed, 0.0f, 0.7f);

    if ((player->stateFlags3 & PLAYER_STATE3_3) || (Player_GetMask(play) == PLAYER_MASK_SKULL))
        Math_ScaledStepToS(&this->unk_150, Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos), 0x400);
    else if ((player->stateFlags2 & PLAYER_STATE2_7) || (player->actor.freezeTimer > 0))
        Math_ScaledStepToS(&this->unk_150, this->actor.yawTowardsPlayer + 0x8000, 0x400);
    else Math_ScaledStepToS(&this->unk_150, this->actor.yawTowardsPlayer, 0x400);

    this->actor.shape.rot.y = (s32)(Math_SinF(this->unk_14E * ((2 * M_PI) / 15)) * (614.4f * this->actor.speed)) + this->unk_150;
    Actor_PlaySfx_Flagged(&this->actor, NA_SE_EN_KUROSUKE_MOVE - SFX_FLAG);

    if (sp20) {
        this->unk_14B &= ~2;
        func_80A4E190(this);
    } else if (this->unk_149 == 0 && !(player->stateFlags3 & PLAYER_STATE3_3) && Player_GetMask(play) != PLAYER_MASK_SKULL && (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) && Actor_IsFacingPlayer(&this->actor, 0x1800) && this->actor.xzDistToPlayer < 120.0f && fabsf(this->actor.yDistToPlayer) < 100.0f)
        func_80A4E58C(this);
}
...
void func_80A4E72C(EnMkk* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        if (this->actor.velocity.y > -1.0f) {
            Color_RGBA8 prim = sEffPrimColors[this->actor.params];
            Color_RGBA8 env = sEffEnvColors[this->actor.params];
            
            temp.x = this->actor.world.pos.x;
            temp.y = this->actor.world.pos.y + 15.0f;
            temp.z = this->actor.world.pos.z;
            EffectSsDeadDb_Spawn(play, &temp, &gZeroVec3f, &gZeroVec3f, 0x46, 4, prim.r, prim.g, prim.b, prim.a, env.r, env.g, env.b, 0xC, 0xC, false);
...
void func_80A4E84C(EnMkk* this) {
    if (this->unk_14B & 3) {
        ...
        if (temp_fv0 < 0.001f)
            ...
            this->unk_154.x = ((this->actor.prevPos.x - this->actor.world.pos.x) * temp_fv0 * 10.0f) + this->actor.world.pos.x;
            this->unk_154.y = ((this->actor.prevPos.y - this->actor.world.pos.y) * temp_fv0 * 10.0f) + this->actor.world.pos.y;
            this->unk_154.z = ((this->actor.prevPos.z - this->actor.world.pos.z) * temp_fv0 * 10.0f) + this->actor.world.pos.z;
        }
        ...
        this->unk_154.x = this->actor.world.pos.x - 10.0f * Math_SinS(this->actor.shape.rot.y + (s32)(1228.8f * this->actor.speed * Math_SinF(this->unk_14E * (M_PI / 5))));
        this->unk_154.z = this->actor.world.pos.z - 10.0f * Math_CosS(this->actor.shape.rot.y + (s32)(1228.8f * this->actor.speed * Math_SinF(this->unk_14E * (M_PI / 5))));
        this->unk_160.x = this->unk_154.x - 12.0f * Math_SinS(this->actor.shape.rot.y - (s32)(1228.8f * this->actor.speed * Math_SinF(this->unk_14E * (M_PI / 5))));
        this->unk_160.z = this->unk_154.z - 12.0f * Math_CosS(this->actor.shape.rot.y - (s32)(1228.8f * this->actor.speed * Math_SinF(this->unk_14E * (M_PI / 5))));
    }
}
...
void EnMkk_Update(Actor* thisx, PlayState* play) {
    ...
    Actor_UpdateBgCheckInfo(play, &this->actor, 20.0f, 20.0f, 20.0f, UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_2 | UPDBGCHECKINFO_FLAG_3 | UPDBGCHECKINFO_FLAG_4);
    ...
    this->collider.elements[0].dim.worldSphere.center.x = this->actor.focus.pos.x;
    this->collider.elements[0].dim.worldSphere.center.y = this->actor.focus.pos.y;
    this->collider.elements[0].dim.worldSphere.center.z = this->actor.focus.pos.z;
...
void func_80A4EE48(EnMkk* this, PlayState* play) {
    if (this->unk_14E > 0) {
        ...
        if (this->unk_14E == 0)
            this->actor.flags &= ~ACTOR_FLAG_UPDATE_CULLING_DISABLED;
    } else if (this->actor.xzDistToPlayer < this->unk_178 && Player_GetMask(play) != PLAYER_MASK_SKULL) {
...
void EnMkk_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.projectedPos.z > 0.0f) {
        ...
        OPEN_DISPS(play->state.gfxCtx, "../z_en_mkk.c", 486);
        ...
        if (this->alpha == 255) {
            ...
            MATRIX_FINALIZE_AND_LOAD(&gfx[3], play->state.gfxCtx, "../z_en_mkk.c", 495);
        ...
        MATRIX_FINALIZE_AND_LOAD(&gfx[3], play->state.gfxCtx, "../z_en_mkk.c", 504);
        ...
        if (thisx->params == 0) {
            ...
            MATRIX_FINALIZE_AND_LOAD(&gfx[5], play->state.gfxCtx, "../z_en_mkk.c", 513);
            ...
            Matrix_Scale(0.85f, 0.85f, 0.85f, MTXMODE_APPLY);
            MATRIX_FINALIZE_AND_LOAD(&gfx[7], play->state.gfxCtx, "../z_en_mkk.c", 521);
...
void func_80A4F4C8(Actor* thisx, PlayState* play) {
    ...
    OPEN_DISPS(play->state.gfxCtx, "../z_en_mkk.c", 541);
    ...
    gSPDisplayList(&gfx[2], modelInfo->materialDL);
    Matrix_ReplaceRotation(&play->billboardMtxF);
    MATRIX_FINALIZE_AND_LOAD(&gfx[3], play->state.gfxCtx, "../z_en_mkk.c", 548);
    gSPDisplayList(&gfx[4], modelInfo->modelDL);
    ...
    MATRIX_FINALIZE_AND_LOAD(&gfx[5], play->state.gfxCtx, "../z_en_mkk.c", 556);
    gSPDisplayList(&gfx[6], modelInfo->modelDL);
    ...
    MATRIX_FINALIZE_AND_LOAD(&gfx[7], play->state.gfxCtx, "../z_en_mkk.c", 563);
    gSPDisplayList(&gfx[8], modelInfo->modelDL);
    ...
    MATRIX_FINALIZE_AND_LOAD(&gfx[9], play->state.gfxCtx, "../z_en_mkk.c", 570);
    gSPDisplayList(&gfx[10], modelInfo->modelDL);
    gSPDisplayList(&gfx[11], modelInfo->endDL);
    POLY_XLU_DISP = &gfx[12];

    CLOSE_DISPS(play->state.gfxCtx, "../z_en_mkk.c", 575);
}
