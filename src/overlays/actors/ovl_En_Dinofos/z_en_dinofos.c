...
#define DODGES (DMG_ARROW_NORMAL | DMG_SPIN_KOKIRI | DMG_SPIN_MASTER | DMG_SPIN_GIANT | DMG_JUMP_KOKIRI | DMG_JUMP_MASTER | DMG_JUMP_GIANT)
...
void EnDinofos_SetupChooseJump(EnDinofos* this, s8 jumpType);
void EnDinofos_SetupDodgeProjectile(EnDinofos* this, s16 angleToProjectile);
void EnDinofos_SetupIdle(EnDinofos* this);
void EnDinofos_DoNothing(EnDinofos* this, PlayState* play);
...
static DamageTable sDamageTable = {
    ...
    /* Deku stick    */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Slingshot     */ DMG_ENTRY(1, DINOLFOS_DMGEFF_NONE),
    /* Explosive     */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Boomerang     */ DMG_ENTRY(0, DINOLFOS_DMGEFF_STUN),
    /* Normal arrow  */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Hammer swing  */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Hookshot      */ DMG_ENTRY(0, DINOLFOS_DMGEFF_IMMUNE),
    /* Kokiri sword  */ DMG_ENTRY(1, DINOLFOS_DMGEFF_NONE),
    /* Master sword  */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Giant's Knife */ DMG_ENTRY(4, DINOLFOS_DMGEFF_NONE),
    /* Fire arrow    */ DMG_ENTRY(2, DINOLFOS_DMGEFF_FIRE),
    /* Ice arrow     */ DMG_ENTRY(2, DINOLFOS_DMGEFF_ICE),
    /* Light arrow   */ DMG_ENTRY(2, DINOLFOS_DMGEFF_LIGHT_SPARKS),
    /* Unk arrow 1   */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Unk arrow 2   */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Unk arrow 3   */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Fire magic    */ DMG_ENTRY(2, DINOLFOS_DMGEFF_FIRE),
    /* Ice magic     */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Light magic   */ DMG_ENTRY(4, DINOLFOS_DMGEFF_NONE),
    /* Shield        */ DMG_ENTRY(0, DINOLFOS_DMGEFF_NONE),
    /* Mirror Ray    */ DMG_ENTRY(0, DINOLFOS_DMGEFF_NONE),
    /* Kokiri spin   */ DMG_ENTRY(1, DINOLFOS_DMGEFF_NONE),
    /* Giant spin    */ DMG_ENTRY(4, DINOLFOS_DMGEFF_NONE),
    /* Master spin   */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Kokiri jump   */ DMG_ENTRY(2, DINOLFOS_DMGEFF_NONE),
    /* Giant jump    */ DMG_ENTRY(8, DINOLFOS_DMGEFF_NONE),
    /* Master jump   */ DMG_ENTRY(4, DINOLFOS_DMGEFF_NONE),
    /* Unknown 1     */ DMG_ENTRY(0, DINOLFOS_DMGEFF_NONE),
    /* Unblockable   */ DMG_ENTRY(0, DINOLFOS_DMGEFF_NONE),
    /* Hammer jump   */ DMG_ENTRY(4, DINOLFOS_DMGEFF_NONE),
    /* Unknown 2     */ DMG_ENTRY(0, DINOLFOS_DMGEFF_NONE),
};

static CollisionCheckInfoInit sColChkInfoInit = { 20, 40, 100, 80 };
...
static InitChainEntry sInitChain[] = {
    ...
    ICHAIN_S8(naviEnemyId, NAVI_ENEMY_ARMORED_DINOLFOS, ICHAIN_CONTINUE),
    ICHAIN_F32_DIV1000(gravity, -2000, ICHAIN_CONTINUE),
    ICHAIN_VEC3F_DIV1000(scale, 15, ICHAIN_STOP),
};

#define EFFECT_BLURE_ELEMENT_FLAG_8         (1 << 3)
#define EFF_BLURE_DRAW_MODE_SMOOTH          2
...
void EnDinofos_ChooseAction(EnDinofos* this, PlayState* play) {
    if (EnDinofos_IsFacingPlayer(this)) {
        if (this->actor.xzDistToPlayer < 100.0f) {
            ...
            else EnDinofos_SetupCircleAroundPlayer(this, play);
        }
...
void EnDinofos_EnableBumperCollision(EnDinofos* this) {
    ...
    if (this->isDodging) {
        for (i=0; i<DINOFOS_COLLIDER_FIRE_START_INDEX; i++)
            this->bodyAndFireCollider.elements[i].base.acDmgInfo.dmgFlags |= DODGES;
        this->isDodging = false;
    }
}
...
bool EnDinofos_Dodge(EnDinofos* this, PlayState* play) {
    ...
    actor = Actor_GetProjectileActor(play, &this->actor, 80.0f);
    if (actor != NULL) {
    ...
    actor = Actor_FindNearby(play, &this->actor, -1, ACTORCAT_EXPLOSIVE, 80.0f);
    ...
    if ((this->actor.xzDistToPlayer < 100.0f) && (player->meleeWeaponState != 0) &&
        ...
        else EnDinofos_SetupChooseJump(this, DINOFOS_JUMP_TYPE_SLASH);
        Actor_PlaySfx(&this->actor, NA_SE_EN_RIZA_WARAU);
        return true;
    }
...
void EnDinofos_DoNothing(EnDinofos* this, PlayState* play) {
    ...
    if (getAggressive)
        this->actionFunc = EnDinofos_Idle;
    else if (this->actor.xzDistToPlayer < 400.0f) { // Player noticed, get active
        if (this->switchFlag != 0xFF) {
...
void EnDinofos_Idle(EnDinofos* this, PlayState* play) {
    ...
    if (this->actionTimer != 0)
        this->actionTimer--;
    else EnDinofos_ChooseAction(this, play);
}
...
void EnDinofos_Walk(EnDinofos* this, PlayState* play) {
    ...
    if (!EnDinofos_Dodge(this, play)) {
        ...
        else Math_StepToF(&this->actor.speed, 7.0f, 0.5f);
...
void EnDinofos_SetupTurnToPlayer(EnDinofos* this) {
    ...
    else this->targetRotY = BINANG_SUB(this->actor.shape.rot.y, 0x4000);
...
void EnDinofos_SetupCircleAroundPlayer(EnDinofos* this, PlayState* play) {
    ...
    if (this->actionFunc != EnDinofos_CircleAroundPlayer) {
        ...
        else Animation_Change(&this->skelAnime, &gDinolfosSidestepAnim, this->actor.speed * (1.0f / 6.0f), 0.0f, 0.0f, ANIMMODE_LOOP, -4.0f);
...
void EnDinofos_CircleAroundPlayer(EnDinofos* this, PlayState* play) {
    ...
    if (!EnDinofos_Dodge(this, play)) {
        if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
            ...
            else angle = BINANG_SUB(this->actor.shape.rot.y, 0x4000);
        ...
        else this->skelAnime.playSpeed = -1.0f;
        ...
        if (this->actor.xzDistToPlayer <= (70.0f + targetDistAdjustment))
            ...
            Math_StepToF(&this->circlingRate, 4.0f, 1.5f);
        else Math_StepToF(&this->circlingRate, 0.0f, 5.65f);
...
void EnDinofos_SetupLand(EnDinofos* this) {
    ...
    else this->skelAnime.endFrame = Animation_GetLastFrame(&gDinolfosAttackAnim);

    if (this->actor.speed < 0.0f)
        this->isJumpingBackward = true;
    else this->isJumpingBackward = false;
...
void func_800BE5CC(Actor* actor, ColliderJntSph* collider, s8 colliderIndex) {
    if (collider->elements[colliderIndex].base.acHitElem->atDmgInfo.dmgFlags & (0x10000 | 0x2000 | 0x1000 | 0x800 | 0x20))
        actor->world.rot.y = collider->base.ac->shape.rot.y;
    else actor->world.rot.y = Actor_WorldYawTowardActor(collider->base.ac, actor);
}

void EnDinofos_SetupDamaged(EnDinofos* this, s8 colliderIndex) {
...
void EffectSsDinoFire_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep, s16 alpha, s16 fadeDelay, s32 life) {
    ...
    EffectSs_Spawn(play, EFFECT_SS_DINOFOS_FIRE, 128, &initParams);
}
...
void EnDinofos_BreatheFire(EnDinofos* this, PlayState* play) {
    ...
    EffectSsDinoFire_Spawn(play, &this->bodyPartsPos[DINOFOS_BODYPART_JAW], &velocity, &accel, 30, 22, 255 - (dimAlphaStep * 20), dimAlphaStep + 20, 8);
    ...
    for (i=DINOFOS_COLLIDER_FIRE_START_INDEX; i<end; i++) {
        dim = &this->bodyAndFireCollider.elements[i].dim;
        fireRotY = (s32)(Math_CosF((this->attackTimer + ((i - 5) << 1)) * (M_PI / 20)) * 0x2C00) + this->actor.shape.rot.y;

        dim->worldSphere.center.x = (s32)this->bodyPartsPos[DINOFOS_BODYPART_JAW].x + (s32)(Math_SinS(fireRotY) * dim->modelSphere.center.z);
        dim->worldSphere.center.y = (s32)this->bodyPartsPos[DINOFOS_BODYPART_JAW].y + (s32)dim->modelSphere.center.y;
        dim->worldSphere.center.z = (s32)this->bodyPartsPos[DINOFOS_BODYPART_JAW].z + (s32)(Math_CosS(fireRotY) * dim->modelSphere.center.z);
    }
...
void EnDinofos_Die(EnDinofos* this, PlayState* play) {
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        else this->envColorAlpha = envColorAlpha;
...
void EnDinofos_SetupDodgeProjectile(EnDinofos* this, s16 angleToProjectile) {
    ...
    Animation_Change(&this->skelAnime, &gDinolfosSidestepAnim, this->actor.speed * (1.0f / 7.5f), 0.0f, 0.0f, ANIMMODE_LOOP, -4.0f);
    this->actor.world.rot.y = BINANG_ADD(this->actor.shape.rot.y, 0x4000);
...
void EnDinofos_DodgeProjectile(EnDinofos* this, PlayState* play) {
    ...
    this->skelAnime.playSpeed = (1.0f + fabsf(this->actor.speed * (1.0f / 15.0f))) * ((this->actor.speed >= 0.0f) ? 1.0f : -1.0f);
    this->actor.world.rot.y = BINANG_ADD(this->actor.shape.rot.y, 0x4000);
    SkelAnime_Update(&this->skelAnime);
    if (this->sidestepTimer != 0)
        this->sidestepTimer--;
...
void EnDinofos_RotateHead(EnDinofos* this, PlayState* play) {
    ...
    if (this->actionFunc == EnDinofos_Idle && this->headTimer != 0)
        ...
        Math_ScaledStepToS(&this->headRotY, Math_CosF(M_PI) * 0x2C00, 0x2C00 / 20);
    else if (this->actionFunc == EnDinofos_BreatheFire)
        this->headRotY = Math_CosF(this->headTimer * (M_PI / 20)) * 0x2C00;
...
bool EnDinofos_UpdateDamage(EnDinofos* this, PlayState* play) {
    ...
    if (this->bodyAndFireCollider.base.acFlags & AC_HIT) {
        ...
        Actor_SetDropFlagJntSph(&this->actor, &this->bodyAndFireCollider, true);
      ...
      //if (this->drawDmgEffType == ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX && (this->bodyAndFireCollider.elements[i].base.acHitElem->atDmgInfo.dmgFlags & 0xDB0B3))
      //    return false;

        if (this->actor.colChkInfo.damageReaction == DINOLFOS_DMGEFF_IMMUNE)
            return false;
        ...
        this->knifeCollider.base.atFlags &= ~(AT_ON | AT_BOUNCED);
        this->bodyAndFireCollider.base.atFlags &= ~AT_ON;
        if (this->actor.colChkInfo.damageReaction == DINOLFOS_DMGEFF_ICE) {
        ...
        if (this->actor.colChkInfo.damageReaction == DINOLFOS_DMGEFF_STUN) {
            this->stunTimer = 40;
        ...
        if (this->actor.colChkInfo.damageReaction == DINOLFOS_DMGEFF_ZORA_MAGIC) {
            this->stunTimer = 40;
            Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_BLUE, 255, COLORFILTER_BUFFLAG_OPA, 40);
            this->drawDmgEffScale = 0.55f;
        ...
        if (this->actor.colChkInfo.damageReaction == DINOLFOS_DMGEFF_FIRE) {
            this->drawDmgEffAlpha = 4.0f;
            this->drawDmgEffScale = 0.55f;
            this->drawDmgEffType = ACTOR_DRAW_DMGEFF_FIRE;
        }
        else if (this->actor.colChkInfo.damageReaction == DINOLFOS_DMGEFF_LIGHT_SPARKS) {
            this->drawDmgEffAlpha = 4.0f;
            this->drawDmgEffScale = 0.55f;
...
void EnDinofos_Update(Actor* thisx, PlayState* play2) {
    ...
    Actor_UpdateBgCheckInfo(play, &this->actor, 25.0f, 30.0f, 60.0f, UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_2 | UPDBGCHECKINFO_FLAG_3 | UPDBGCHECKINFO_FLAG_4 | UPDBGCHECKINFO_FLAG_6);
    if (this->actionFunc != EnDinofos_Stunned) {
        if ((this->actor.depthInWater > 0.0f) && (this->actor.depthInWater < 10.0f)) {
            if (((play->gameplayFrames % 4) & 1) == 0) {
                Math_Vec3f_Copy(&bodyPartPos, &this->bodyPartsPos[DINOFOS_BODYPART_LEFT_CLAWS + (play->gameplayFrames % 4)]);
...
s32 EnDinofos_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx) {
    EnDinofos* this = ((EnDinofos*)thisx);
...
void EnDinofos_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfx) {
    EnDinofos* this = (EnDinofos*)thisx;
    ...
    if ((limbIndex == DINOLFOS_LIMB_RIGHT_HAND) && (this->timer2 != this->attackTimer) &&
        ...
        Collider_SetQuadVertices(&this->knifeCollider, &knifeBaseQuadPos, &knifeTipQuadPos, &prevKnifeBaseQuadPos, &prevKnifeTipQuadPos);
        if (this->knifeCollider.base.atFlags & AT_ON)
...
void EnDinofos_Draw(Actor* thisx, PlayState* play) {
    ...
    OPEN_DISPS(play->state.gfxCtx, "../z_en_dinofos.c", 1558);
    ...
    if (this->envColorAlpha == 255) {
        ...
        POLY_OPA_DISP = SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount, EnDinofos_OverrideLimbDraw, EnDinofos_PostLimbDraw, &this->actor, POLY_OPA_DISP);
    }
    else {
        ...
        POLY_XLU_DISP = SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount, EnDinofos_OverrideLimbDraw, EnDinofos_PostLimbDraw, &this->actor, POLY_XLU_DISP);
    }

    Actor_DrawDamageEffects(play, &this->actor, this->bodyPartsPos, DINOFOS_BODYPART_MAX, this->drawDmgEffScale, this->drawDmgEffFrozenSteamScale, this->drawDmgEffAlpha, this->drawDmgEffType);

    CLOSE_DISPS(play->state.gfxCtx, "../z_en_dinofos.c", 1587);
}
