/**
 * Somaria Cubes - Elegy statues from Cane of Byrna/Somaria
 * Uses MM Elegy of Emptiness shell models
 * Originally based off from En_Lightbox
 *
...
void ObjSomariaCube_Update(Actor* thisx, PlayState* play) {
    ...
    switch (state) {
        ...
        case SOMARIA_STATE_IDLE:
            ...
            if ((thisx->bgCheckFlags & BGCHECKFLAG_GROUND) && thisx->speed == 0.0f)
                Actor_UpdateBgCheckInfo(play, thisx, 30.0f, 15.0f, 0.0f, 0x1D);
            if (!(thisx->bgCheckFlags & BGCHECKFLAG_GROUND) || thisx->speed != 0.0f) {
                Actor_MoveXZGravity(thisx);
                Actor_UpdateBgCheckInfo(play, thisx, 30.0f, 15.0f, 0.0f, 0x1D);
    ...
    if (slot >= 0) {
        ...
        if (state != SOMARIA_STATE_HELD && state != SOMARIA_STATE_SPAWN)
            CollisionCheck_SetOC(play, &play->colChkCtx, &sColliderPool[slot].collider.base);
    }
}
