...
void BossTw_TwinrovaUpdate(Actor* thisx, PlayState* play2) {
    ...
    if (this->work[INVINC_TIMER] == 0) {
        if (this->actionFunc != BossTw_TwinrovaStun) {
            ...
            this->collider.base.acFlags &= ~AC_HIT;
            swordDamage = false;
            damage = CollisionCheck_GetSwordDamage(acHitElem->atDmgInfo.dmgFlags, this->actor.colChkInfo.defense);
            ...
            if (!(acHitElem->atDmgInfo.dmgFlags & DMG_HOOKSHOT)) {
                if (((s16)this->actor.colChkInfo.health < Actor_EnemyHealthCheckMultiply(3)) && !swordDamage) {
                    damage = 0;
                }
    ...
    if (sFreezeState == 1) {
        ...
        Sfx_PlaySfxAtPos(&player->actor.projectedPos, LINK_IS_ADULT ? NA_SE_VO_LI_FREEZE : NA_SE_VO_LI_FREEZE_KID);
...
void BossTw_TwinrovaChargeBlast(BossTw* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, this->workf[ANIM_SW_TGT])) {
        if ((s16)this->actor.colChkInfo.health < Actor_EnemyHealthMultiply(10, BOSS_HP)) {
