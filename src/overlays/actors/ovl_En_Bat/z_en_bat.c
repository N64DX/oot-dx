...
void EnBat_Init(Actor* thisx, struct PlayState* play);
void EnBat_Destroy(Actor* thisx, struct PlayState* play);
void EnBat_Update(Actor* thisx, struct PlayState* play);
void EnBat_Draw(Actor* thisx, struct PlayState* play);

void EnBat_SetupPerch(EnBat* this);
void EnBat_Perch(EnBat* this, struct PlayState* play);
void EnBat_SetupFlyIdle(EnBat* this);
void EnBat_FlyIdle(EnBat* this, struct PlayState* play);
void EnBat_SetupDiveAttack(EnBat* this);
void EnBat_DiveAttack(EnBat* this, struct PlayState* play);
void EnBat_Die(EnBat* this, struct PlayState* play);
void EnBat_Stunned(EnBat* this, struct PlayState* play);
...
static DamageTable sDamageTable = {
    ...
    /* Deku stick    */ DMG_ENTRY(2, BAD_BAT_DMGEFF_NONE),
    /* Slingshot     */ DMG_ENTRY(1, BAD_BAT_DMGEFF_NONE),
    /* Explosive     */ DMG_ENTRY(2, BAD_BAT_DMGEFF_NONE),
    /* Boomerang     */ DMG_ENTRY(1, BAD_BAT_DMGEFF_NONE),
    /* Normal arrow  */ DMG_ENTRY(2, BAD_BAT_DMGEFF_NONE),
    /* Hammer swing  */ DMG_ENTRY(2, BAD_BAT_DMGEFF_NONE),
    /* Hookshot      */ DMG_ENTRY(1, BAD_BAT_DMGEFF_NONE),
    /* Kokiri sword  */ DMG_ENTRY(1, BAD_BAT_DMGEFF_NONE),
    /* Master sword  */ DMG_ENTRY(2, BAD_BAT_DMGEFF_NONE),
    /* Giant's Knife */ DMG_ENTRY(4, BAD_BAT_DMGEFF_NONE),
    ...
    /* Ice magic     */ DMG_ENTRY(3, BAD_BAT_DMGEFF_NONE),
    /* Light magic   */ DMG_ENTRY(0, BAD_BAT_DMGEFF_NONE),
    /* Shield        */ DMG_ENTRY(0, BAD_BAT_DMGEFF_NONE),
    /* Mirror Ray    */ DMG_ENTRY(0, BAD_BAT_DMGEFF_NONE),
    /* Kokiri spin   */ DMG_ENTRY(1, BAD_BAT_DMGEFF_NONE),
    /* Giant spin    */ DMG_ENTRY(4, BAD_BAT_DMGEFF_NONE),
    /* Master spin   */ DMG_ENTRY(2, BAD_BAT_DMGEFF_NONE),
    /* Kokiri jump   */ DMG_ENTRY(2, BAD_BAT_DMGEFF_NONE),
    /* Giant jump    */ DMG_ENTRY(8, BAD_BAT_DMGEFF_NONE),
    /* Master jump   */ DMG_ENTRY(4, BAD_BAT_DMGEFF_NONE),
    /* Unknown 1     */ DMG_ENTRY(0, BAD_BAT_DMGEFF_NONE),
    /* Unblockable   */ DMG_ENTRY(0, BAD_BAT_DMGEFF_NONE),
    /* Hammer jump   */ DMG_ENTRY(4, BAD_BAT_DMGEFF_NONE),
...
static Gfx* sWingsDLs[] = {
    gBadBatWingsFrame0DL, gBadBatWingsFrame1DL, gBadBatWingsFrame2DL, gBadBatWingsFrame3DL, gBadBatWingsFrame4DL, gBadBatWingsFrame5DL, gBadBatWingsFrame6DL, gBadBatWingsFrame7DL, gBadBatWingsFrame8DL,
};

#define BAD_BAT_MAX_NUMBER_ATTACKING 3

u8 sNumberAttacking;    // Limit number attacking player to at most `BAD_BAT_MAX_NUMBER_ATTACKING`
u8 sEnBatAlreadySpawned;   // used for those spawned with room -1 in Graveyard to avoid respawn on room change
...
void EnBat_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->paramFlags & BAD_BAT_PARAMFLAG_PERCH) {
        ...
        while (BAD_BAT_GET_NUMBER_TO_SPAWN(thisx) > 1) {
            Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_EN_BAT, thisx->world.pos.x + Rand_CenteredFloat(200.0f), thisx->world.pos.y + Rand_CenteredFloat(100.0f), thisx->world.pos.z + Rand_CenteredFloat(200.0f), Rand_CenteredFloat(0x2000), 0xFFFF * Rand_ZeroOne(), 0, BAD_BAT_PARAMS(this->switchFlag, this->paramFlags, 0));
...
void EnBat_FlyIdle(EnBat* this, PlayState* play) {
    ...
    } else if (Math3D_Dist2DSq(this->actor.world.pos.x, this->actor.world.pos.z, this->actor.home.pos.x, this->actor.home.pos.z) > SQ(300.0f))
        this->yawTarget = Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos);
    else if (finishedRotStep && (Rand_ZeroOne() < 0.015f))
        this->yawTarget = this->actor.shape.rot.y + (((s32)(0x1000 * Rand_ZeroOne()) + 0x1000) * ((Rand_ZeroOne() < 0.5f) ? -1 : 1));
    ...
    if (this->timer != 0)
        this->timer--;
    if (this->actor.xzDistToPlayer < 300.0f && this->timer == 0 && Player_GetMask(play) != PLAYER_MASK_SKULL && sNumberAttacking < BAD_BAT_MAX_NUMBER_ATTACKING && (!(this->paramFlags & BAD_BAT_PARAMFLAG_CHECK_HEIGHTREL) || fabsf(this->actor.yDistToPlayer) < 150.0f))
...
void EnBat_DiveAttack(EnBat* this, PlayState* play) {
    ...
    if (this->timer == 0 || (this->collider.base.atFlags & AT_HIT) || Player_GetMask(play) == PLAYER_MASK_SKULL || (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) || (player->stateFlags1 & PLAYER_STATE1_23) || this->actor.depthInWater > -40.0f) {
        ...
        sNumberAttacking--;
        EnBat_SetupFlyIdle(this);
    } else if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) && ABS_ALT(BINANG_SUB(this->actor.wallYaw, this->actor.yawTowardsPlayer)) > 0x6800) {
        sNumberAttacking--;
        this->collider.base.atFlags &= ~AT_ON;
        this->actor.bgCheckFlags &= ~BGCHECKFLAG_WALL;
        this->yawTarget = this->actor.wallYaw;
...
void EnBat_SetupDie(EnBat* this, PlayState* play) {
    ...
    if (this->actor.colChkInfo.damageReaction == BAD_BAT_DMGEFF_ICE) {
        this->drawDmgEffType = ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX;
        this->drawDmgEffAlpha = 1.0f;
        this->drawDmgEffFrozenSteamScale = 60.0f * (0.45f / 40.0f);
        this->drawDmgEffScale = 0.45f;
    } else if (this->actor.colChkInfo.damageReaction == BAD_BAT_DMGEFF_LIGHT) {
        this->drawDmgEffType = ACTOR_DRAW_DMGEFF_LIGHT_ORBS;
        this->drawDmgEffAlpha = 4.0f;
        this->drawDmgEffScale = 0.45f;
    } else if (this->actor.colChkInfo.damageReaction == BAD_BAT_DMGEFF_FIRE) {
...
void EnBat_Die(EnBat* this, PlayState* play) {
    ...
    if (!(this->actor.flags & ACTOR_FLAG_ATTACHED_TO_ARROW)) {
        ...
        if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) || (this->actor.floorHeight == BGCHECK_Y_MIN)) {
            ...
            EffectSsDeadDb_Spawn(play, &this->actor.world.pos, &gZeroVec3f, &gZeroVec3f, 100, 0, 255, 255, 255, 255, 255, 0, 0, 1, 9, true);
...
void EnBat_UpdateDamage(EnBat* this, PlayState* play) {
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if (this->actor.colChkInfo.damageReaction == BAD_BAT_DMGEFF_ELECTRIC) {
            this->timer = 40;
            this->drawDmgEffType = ACTOR_DRAW_DMGEFF_ELECTRIC_SPARKS_LARGE;
            this->drawDmgEffAlpha = 2.0f;
            this->drawDmgEffScale = 0.45f;
            EnBat_SetupStunned(this);
        } else if (this->actor.colChkInfo.damageReaction == BAD_BAT_DMGEFF_STUN) {
            this->timer = 40;
            EnBat_SetupStunned(this);
        } else {
...
void EnBat_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != EnBat_Perch) {
        ...
        else Actor_MoveXZGravity(&this->actor);
