...
// PARAMFLAGS
#define BAD_BAT_PARAMFLAG_0                 (1 << 0) // Affects which flags are used in Actor_UpdateBgCheckInfo in certain circumstances
#define BAD_BAT_PARAMFLAG_CHECK_HEIGHTREL   (1 << 1) // Whether to apply a height range check to attack
#define BAD_BAT_PARAMFLAG_PERCH             (1 << 2) // Spawn as perching instead of flying    

struct EnBat;

typedef void (*EnBatActionFunc)(struct EnBat*, struct PlayState*);
...
typedef struct EnBat {
    ...
    /* 0x184 */ ColliderJntSph collider;
    /* 0x1B4 */ ColliderJntSphElement colliderElements[1];
} EnBat; // size = 0x1E4
