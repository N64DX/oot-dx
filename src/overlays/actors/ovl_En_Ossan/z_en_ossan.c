...
void EnOssan_DrawCursor(PlayState* play, EnOssan* this, f32 x, f32 y, f32 z, u8 drawCursor) {
    ...
    if (drawCursor != 0) {
        ...
        w = 16.0f * z;
        ulx = X_MULTIPLY(HIRES_MULTIPLY((x - w) * 4.0f));
        uly = HIRES_MULTIPLY((y - w) * 4.0f);
        lrx = X_MULTIPLY(HIRES_MULTIPLY((x + w) * 4.0f));
        lry = HIRES_MULTIPLY((y + w) * 4.0f);
        dsdx = (1.0f / z) * 1024.0f;
        gSPTextureRectangle(OVERLAY_DISP++, ulx, uly, lrx, lry, G_TX_RENDERTILE, 0, 0, X_DIVIDE(HIRES_DIVIDE(dsdx)), HIRES_DIVIDE(dsdx));
    }
...
void EnOssan_DrawTextRec(PlayState* play, s32 r, s32 g, s32 b, s32 a, f32 x, f32 y, f32 z, s32 s, s32 t, f32 dx,
    ...
    gSPTextureRectangle(OVERLAY_DISP++, ulx, uly, lrx, lry, G_TX_RENDERTILE, s, t, X_DIVIDE(HIRES_DIVIDE(dsdx)), HIRES_DIVIDE(dtdy));
...
void EnOssan_DrawBazaarShopkeeper(Actor* thisx, PlayState* play) {
    ...
    EnOssan_DrawCursor(play, this,  X_DIVIDE(HIRES_DIVIDE(this->cursorX)), HIRES_DIVIDE(this->cursorY), this->cursorZ, this->drawCursor);
