...
#include "assets/objects/object_kgy/object_kgy.h"
...
void EnOssan_DrawSmithyShopkeeper(Actor* thisx, PlayState* play);
...
void EnOssan_InitSmithyShopkeeper(EnOssan* this, PlayState* play);
...
static ShopItem sShopkeeperStores[][8] = {
    ...
    { { SI_DEKU_SHIELD_REPAIR, 50, 52, -20 },
      { SI_HEROS_SHIELD_REPAIR, 50, 76, -20 },
      { SI_HYLIAN_SHIELD_REPAIR, 80, 52, -3 },
      { SI_MIRROR_SHIELD_REPAIR, 80, 76, -3 },
      { SI_HYLIAN_SHIELD_UPGRADE, -50, 52, -20 },
      { SI_MIRROR_SHIELD_UPGRADE, -50, 76, -20 },
...
void EnOssan_TalkBazaarShopkeeper2(PlayState* play) {
    Message_ContinueTextbox(play, 0x8011);
}
...
void EnOssan_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->objectSlot1 < 0) {
        Actor_Kill(&this->actor);
        PRINTF_COLOR_ERROR();
        PRINTF(T("バンクが無いよ！！(%s)\n", "There is no bank!! Shopkeeper params: (%s)\n"), this->actor.params);
        PRINTF_RST();
    ...
    if (EnOssan_TryGetObjBankIndices(this, play, objectIds) == 0) {
        Actor_Kill(&this->actor);
        PRINTF_COLOR_ERROR();
        PRINTF(T("予備バンクが無いよ！！(%s)\n", "There is no spare bank!! Shopkeeper params: (%s)\n"),
               this->actor.params);
        PRINTF_RST();
...
void EnOssan_StartShopping(PlayState* play, EnOssan* this) {
    ...
    if (this->actor.params == OSSAN_TYPE_SMITHY && SHIELD_DURABILITY) {
        if (!GET_EVENTCHKINF(EVENTCHKINF_45) && !GET_INFTABLE(INFTABLE_TALKED_TO_SMITHY_PRE_TIME_SKIP))
            SET_INFTABLE(INFTABLE_TALKED_TO_SMITHY_PRE_TIME_SKIP);
        if (GET_EVENTCHKINF(EVENTCHKINF_45)) {
            if (GET_EVENTCHKINF(EVENTCHKINF_CLEANSED_GORON_MINES) && GET_INFTABLE(INFTABLE_TALKED_TO_ZUBORA_MINES) && GET_INFTABLE(INFTABLE_TALKED_TO_SMITHY_GORON_MINES))
                SET_INFTABLE(INFTABLE_TALKED_TO_SMITHY_GORON_MINES);
            if (!GET_EVENTCHKINF(EVENTCHKINF_CLEANSED_GORON_MINES) || !GET_INFTABLE(INFTABLE_TALKED_TO_ZUBORA_MINES))
                SET_INFTABLE(INFTABLE_TALKED_TO_SMITHY_POST_TIME_SKIP);
        }
    }
...
void EnOssan_State_Idle(EnOssan* this, PlayState* play, Player* player) {
    ...
    } else if (this->actor.params == OSSAN_TYPE_SMITHY && this->actor.xzDistToPlayer < 200.0f) {
        Actor_OfferTalk(&this->actor, play, 200);
...
void EnOssan_GiveItemWithFanfare(PlayState* play, EnOssan* this) {
    ...
    Actor_OfferGetItem(&this->actor, play, this->shelfSlots[this->cursorIndex]->getItemId, this->actor.params == OSSAN_TYPE_SMITHY ? 200.0f : 120.0f, this->actor.params == OSSAN_TYPE_SMITHY ? 200.0f : 120.0f);
    play->msgCtx.msgMode = MSGMODE_TEXT_CLOSING;
    play->msgCtx.stateTimer = 4;
    player->stateFlags2 &= ~PLAYER_STATE2_29;
    Play_SetViewpoint(play, VIEWPOINT_LOCKED);
            ...
            if (!EnOssan_TestEndInteraction(this, play, &play->state.input[0])) {
                ...
                switch (play->msgCtx.choiceIndex) {
                    case 0:
                        ...
                        Actor_OfferTalkExchangeEquiCylinder(&this->actor, play, this->actor.params == OSSAN_TYPE_SMITHY ? 200.0f : 100.0f, -1);
                        break;
                    case 1:
                    default:
...
void EnOssan_InitSmithyShopkeeper(EnOssan* this, PlayState* play) {
    static AnimationHeader* sAnimations[] = { &object_kgy_Anim_004B98, &object_kgy_Anim_0008FC, &object_kgy_Anim_00292C, &object_kgy_Anim_0042E4, &object_kgy_Anim_0101F0, &object_kgy_Anim_001764, &object_kgy_Anim_003334, &object_kgy_Anim_010B84, &object_kgy_Anim_001EA4, &object_kgy_Anim_003D88 };
    SkelAnime_InitFlex(play, &this->skelAnime, &object_kgy_Skel_00F910, sAnimations[2], NULL, NULL, 0);
    this->actor.draw = EnOssan_DrawSmithyShopkeeper;
    this->obj3ToSeg6Func = NULL;
}
...
u16 EnOssan_SetupHelloDialog(EnOssan* this) {
    ...
    if (this->actor.params == OSSAN_TYPE_SMITHY) {
        ...
        if (GET_EVENTCHKINF(EVENTCHKINF_45)) {
            if (GET_EVENTCHKINF(EVENTCHKINF_CLEANSED_GORON_MINES) && GET_INFTABLE(INFTABLE_TALKED_TO_ZUBORA_MINES) && GET_INFTABLE(INFTABLE_TALKED_TO_SMITHY_GORON_MINES))
                return INFTABLE_TALKED_TO_SMITHY_PRE_TIME_SKIP ? 0x8302 : 0x8300;
            if (!GET_EVENTCHKINF(EVENTCHKINF_CLEANSED_GORON_MINES) || !GET_INFTABLE(INFTABLE_TALKED_TO_ZUBORA_MINES))
...
void EnOssan_DrawCursor(PlayState* play, EnOssan* this, f32 x, f32 y, f32 z, u8 drawCursor) {
    ...
    if (drawCursor != 0) {
        ...
        w = 16.0f * z;
        ulx = X_HIRES_MULTIPLY((x - w) * 4.0f);
        uly = HIRES_MULTIPLY((y - w) * 4.0f);
        lrx = X_HIRES_MULTIPLY((x + w) * 4.0f);
        lry = HIRES_MULTIPLY((y + w) * 4.0f);
        dsdx = (1.0f / z) * 1024.0f;
        gSPTextureRectangle(OVERLAY_DISP++, ulx, uly, lrx, lry, G_TX_RENDERTILE, 0, 0, X_HIRES_DIVIDE(dsdx), HIRES_DIVIDE(dsdx));
    }
...
void EnOssan_DrawTextRec(PlayState* play, s32 r, s32 g, s32 b, s32 a, f32 x, f32 y, f32 z, s32 s, s32 t, f32 dx,
    ...
    gSPTextureRectangle(OVERLAY_DISP++, ulx, uly, lrx, lry, G_TX_RENDERTILE, s, t, X_HIRES_DIVIDE(dsdx), HIRES_DIVIDE(dtdy));
    ...
    if (drawStickLeftPrompt || drawStickRightPrompt) {
        ...
        if (drawStickRightPrompt) {
            EnOssan_DrawTextRec(play, this->stickRightPrompt.arrowColorR, this->stickRightPrompt.arrowColorG,
                                ...
                                this->stickRightPrompt.arrowTexX + WS_SHIFT_FULL, this->stickRightPrompt.arrowTexY,
        ...
        if (drawStickRightPrompt) {
            EnOssan_DrawTextRec(play, this->stickRightPrompt.stickColorR, this->stickRightPrompt.stickColorG,
                                ...
                                this->stickRightPrompt.stickTexX + WS_SHIFT_FULL, this->stickRightPrompt.stickTexY,
                                this->stickRightPrompt.z, 0, 0, 1.0f, 1.0f);
        }
    }
...
void EnOssan_DrawBazaarShopkeeper(Actor* thisx, PlayState* play) {
    ...
    EnOssan_DrawCursor(play, this, X_HIRES_DIVIDE(this->cursorX), HIRES_DIVIDE(this->cursorY), this->cursorZ, this->drawCursor);
