...
void ObjFlowerpot_Destroy(Actor* thisx, struct PlayState* play2);
void ObjFlowerpot_Update(Actor* thisx, struct PlayState* play2);
void ObjFlowerpot_Draw(Actor* thisx, struct PlayState* play);

void func_80A1C818(ObjFlowerpot* this);
void func_80A1C838(ObjFlowerpot* this, struct PlayState* play);
void func_80A1CBF8(ObjFlowerpot* this);
void func_80A1CC0C(ObjFlowerpot* this, struct PlayState* play);
void func_80A1CD10(ObjFlowerpot* this);
void func_80A1CEF4(ObjFlowerpot* this, struct PlayState* play);
...
void func_80A1BA44(ObjFlowerpot* this, PlayState* play) {
    ...
    for (i=0, phi_s3=0; i<18; i++, phi_s3 += 0xE38) {
        ...
        EffectSsKakera_Spawn(play, &spC4, &spB8, &spC4, -600, phi_s0, 30, 0, 0, (Rand_ZeroOne() * 12.0f) + 16.6f, phi_s1, 0, 35, -1, OBJECT_FLOWERPOT, object_flowerpot_DL_0015B0);
    }

    spD0.y += 20.0f;

    func_80033480(play, &spD0, 30.0f, 2, 20, 50, 1);
    func_80033480(play, &spD0, 30.0f, 2, 10, 80, 1);
    func_80033480(play, &spD0, 30.0f, 1, 10, 40, 1);
}
...
void func_80A1BD80(ObjFlowerpot* this, PlayState* play) {
    ...
    for (i=0, phi_s1=0; i<14; i++, phi_s1 += 0x1249) {
        ...
        EffectSsKakera_Spawn(play, &spBC, &spB0, &spBC, -240, phi_s0, 40, 0, 0, (Rand_ZeroOne() * 20.0f) + 10.6f, 0, 0, 42, -1, OBJECT_FLOWERPOT, object_flowerpot_DL_0015B0);
    }
...
void func_80A1C0FC(ObjFlowerpot* this, PlayState* play) {
    ...
    for (i=0, phi_s0=0; i<10; i++, phi_s0 += 0x1999) {
        ...
        EffectSsKakera_Spawn(play, &spB8, &spAC, &spB8, -100, 64, 40, 0, 0, (Rand_ZeroOne() * 16.0f) + 14.0f, 0, 0, 80, -1, OBJECT_FLOWERPOT, object_flowerpot_DL_0014F0);
    }
}
...
void func_80A1C328(ObjFlowerpot* this, PlayState* play) {
    ...
    for (i=0, phi_s0=0; i<10; i++, phi_s0 += 0x1999) {
        ...
        Math_Vec3f_Sum(&spB8, &spC4, &spB8);
        EffectSsKakera_Spawn(play, &spB8, &spAC, &spB8, -80, 64, 44, 0, 0, (Rand_ZeroOne() * 16.0f) + 14.0f, 0, 0, 80, -1, OBJECT_FLOWERPOT, object_flowerpot_DL_0014F0);
    }
}
...
void func_80A1C554(ObjFlowerpot* this) {
    if ((this->actor.projectedPos.z < 455.0f) && (this->actor.projectedPos.z > -10.0f)) {
        ...
        else this->actor.shape.shadowAlpha = 455 - (s32)this->actor.projectedPos.z;
    } else this->actor.shape.shadowDraw = NULL;
}

void func_80A1C5E8(ObjFlowerpot* this, PlayState* play) {
    Actor_UpdateBgCheckInfo(play, &this->actor, 18.0f, 15.0f, 0.0f, UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_2 | UPDBGCHECKINFO_FLAG_6);
}
...
void ObjFlowerpot_Init(Actor* thisx, PlayState* play) {
    ...
    Matrix_SetTranslateRotateYXZ(this->actor.home.pos.x, this->actor.home.pos.y, this->actor.home.pos.z, &this->actor.shape.rot);
    ...
    D_80A1D400++;
    this->droppedRupeeOnPickup = false;
}
...
void func_80A1C838(ObjFlowerpot* this, PlayState* play) {
    if (Actor_HasParent(&this->actor, play)) {
        ...
        if (dropParams >= 0 && dropParams < ITEM00_MAX && !this->droppedRupeeOnPickup) {
            Item_DropCollectible(play, &this->actor.world.pos, ITEM00_RUPEE_GREEN | ((PARAMS_GET_U(this->actor.params, 9, 6) + 1) << 8));
            this->droppedRupeeOnPickup = true;
        }

        // @bug: This function should only pass Player*: it uses *(this + 0x153), which is meant to be player->currentMask, but in this case is garbage in the collider
    ...
    } else if ((this->collider.elements[0].base.acElemFlags & ACELEM_HIT) && (this->collider.elements[0].base.acHitElem->atDmgInfo.dmgFlags & 0x058BFFBC)) {
        if (!(this->unk_1EA & 2)) {
            func_80A1B914(this, play);
...
void func_80A1CC0C(ObjFlowerpot* this, PlayState* play) {
    ...
    if (Actor_HasNoParent(&this->actor, play)) {
        ...
        this->actor.floorHeight = BgCheck_EntityRaycastDown5(play, &play->colCtx, &this->actor.floorPoly, &bgId, &this->actor, &sp30);
    }
}
...
void func_80A1CEF4(ObjFlowerpot* this, PlayState* play) {
    ...
    if ((thisx->bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_GROUND_TOUCH | BGCHECKFLAG_WALL)) || sp28 || this->unk_1E8 <= 0) {
        if (!(this->unk_1EA & 2)) {
            func_80A1B914(this, play);
            func_80A1C0FC(this, play);
            func_80A1B9CC(this, play);
    ...
    if (thisx->bgCheckFlags & BGCHECKFLAG_WATER_TOUCH) {
        ...
        func_80A1BD80(this, play);
        func_80A1B994(this, play);
        SfxSource_PlaySfxAtFixedWorldPos(play, &thisx->world.pos, 40, NA_SE_EV_DIVE_INTO_WATER_L);
        Actor_Kill(thisx);
        return;
    }
