...
void BgKin2Picture_Init(Actor* thisx, struct PlayState* play);
void BgKin2Picture_Destroy(Actor* thisx, struct PlayState* play);
void BgKin2Picture_Update(Actor* thisx, struct PlayState* play);
void BgKin2Picture_Draw(Actor* thisx, struct PlayState* play);

void BgKin2Picture_SetupWait(BgKin2Picture* this);
void BgKin2Picture_Wait(BgKin2Picture* this, struct PlayState* play);
void BgKin2Picture_SetupShiver(BgKin2Picture* this);
void BgKin2Picture_Shiver(BgKin2Picture* this, struct PlayState* play);
void BgKin2Picture_SetupFall(BgKin2Picture* this);
void BgKin2Picture_Fall(BgKin2Picture* this, struct PlayState* play);
void BgKin2Picture_SetupDoNothing(BgKin2Picture* this);
void BgKin2Picture_DoNothing(BgKin2Picture* this, struct PlayState* play);
...
void BgKin2Picture_SpawnSkulltula(BgKin2Picture* this, PlayState* play2) {
    ...
    if (!BG_KIN2_PICTURE_SKULLTULA_COLLECTED(&this->dyna.actor)) { // Gold Skulltula is still here.
        ...
        if (!BgKin2Picture_IsSkulltulaCollected(play, skulltulaSpawnParams) &&
            Actor_Spawn(&play->actorCtx, play, ACTOR_EN_SW, this->dyna.actor.home.pos.x, this->dyna.actor.home.pos.y + 23.0f, this->dyna.actor.home.pos.z, 0, this->dyna.actor.home.rot.y, 0, skulltulaSpawnParams)) {
...
void BgKin2Picture_SpawnDust(BgKin2Picture* this, PlayState* play) {
    ...
    s32 angle, scale, baseAngle;
    s16 scaleStep;
    u8 i;

    Matrix_SetTranslateRotateYXZ(this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.y + (this->dyna.actor.shape.yOffset * this->dyna.actor.scale.y), this->dyna.actor.world.pos.z, &this->dyna.actor.shape.rot);
...
void BgKin2Picture_Init(Actor* thisx, PlayState* play) {
    ...
    CollisionHeader_GetVirtual(&gOceanSpiderHouseSkullkidPaintingCol, &header);
    ...
    if (BG_KIN2_PICTURE_SKULLTULA_COLLECTED(&this->dyna.actor) || BgKin2Picture_IsSkulltulaCollected(play, skulltulaParams))
...
void BgKin2Picture_Wait(BgKin2Picture* this, PlayState* play) {
    if (this->colliderTris.base.acFlags & AC_HIT) { // hit by projectile
        ...
        if (this->skulltulaNoiseTimer >= 0) {
            if (this->skulltulaNoiseTimer == 0) {
                ...
                else this->skulltulaNoiseTimer = 8;
            } else this->skulltulaNoiseTimer--;
        }
...
void BgKin2Picture_Fall(BgKin2Picture* this, PlayState* play) {
    ...
    } else this->dyna.actor.shape.yOffset = Math_SinS(this->dyna.actor.shape.rot.x) * 40.0f;
}
