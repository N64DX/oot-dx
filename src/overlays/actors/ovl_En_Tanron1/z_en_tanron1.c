...
void EnTanron1_Destroy(Actor* thisx, struct PlayState* play);
void EnTanron1_Update(Actor* thisx, struct PlayState* play);
void EnTanron1_Draw(Actor* thisx, struct PlayState* play);

void func_80BB5318(EnTanron1* this, struct PlayState* play);
void func_80BB5AAC(EnTanron1* this, struct PlayState* play);
...
void EnTanron1_Update(Actor* thisx, PlayState* play) {
    ...
    switch (this->unk_144) {
        ...
        case 100:
            ...
            while (phi_f18 < 1000.0f) {
                temp_a0 = play->actorCtx.actorLists[ACTORCAT_PROP].head;
                while (temp_a0 != NULL) {
    ...
    if (player->heldItemAction == PLAYER_IA_DEKU_STICK && player->unk_860 != 0) {
        this->unk_14C.x = MELEE_WEAPON_INFO_TIP(&player->meleeWeaponInfo[0])->x;
        this->unk_14C.y = MELEE_WEAPON_INFO_TIP(&player->meleeWeaponInfo[0])->y;
        this->unk_14C.z = MELEE_WEAPON_INFO_TIP(&player->meleeWeaponInfo[0])->z;
...
void func_80BB5318(EnTanron1* this, PlayState* play) {
    ...
    if (spB4 != NULL) {
        ...
        if (spB8 >= 20)
            ...
            Actor_PlaySfx(&this->actor, NA_SE_EN_FLAME_RUN - SFX_FLAG);
    }
}
...
void func_80BB5AAC(EnTanron1* this, PlayState* play) {
    ...
    for (i = 0; i < this->actor.params; i++, ptr++) {
        if (ptr->unk_24 == 2) {
            if (!flag) {
                gSPDisplayList(POLY_OPA_DISP++, ovl_En_Tanron1_DL_001888);
                gDPLoadTextureBlock(POLY_OPA_DISP++, ovl_En_Tanron1_DL_001428, G_IM_FMT_RGBA, G_IM_SIZ_16b, 16, 32, 0, G_TX_NOMIRROR | G_TX_CLAMP, G_TX_NOMIRROR | G_TX_CLAMP, 4, 5, G_TX_NOLOD, G_TX_NOLOD);
                flag++;
            }
    ...
    CLOSE_DISPS(play->state.gfxCtx, "../z_en_tanron1.c", 398);
}
