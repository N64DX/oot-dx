...
void BgNumaHana_Destroy(Actor* thisx, struct PlayState* play);
void BgNumaHana_Update(Actor* thisx, struct PlayState* play);
void BgNumaHana_Draw(Actor* thisx, struct PlayState* play2);

void BgNumaHana_SetupDoNothing(BgNumaHana* this);
void BgNumaHana_DoNothing(BgNumaHana* this, struct PlayState* play);
void BgNumaHana_SetupClosedIdle(BgNumaHana* this);
void BgNumaHana_ClosedIdle(BgNumaHana* this, struct PlayState* play);
void BgNumaHana_SetupUnfoldInnerPetals(BgNumaHana* this);
void BgNumaHana_UnfoldInnerPetals(BgNumaHana* this, struct PlayState* play);
void BgNumaHana_SetupUnfoldOuterPetals(BgNumaHana* this);
void BgNumaHana_UnfoldOuterPetals(BgNumaHana* this, struct PlayState* play);
void BgNumaHana_SetupRaiseFlower(BgNumaHana* this);
void BgNumaHana_RaiseFlower(BgNumaHana* this, struct PlayState* play);
void BgNumaHana_SetupOpenedIdle(BgNumaHana* this);
void BgNumaHana_OpenedIdle(BgNumaHana* this, struct PlayState* play);
...
#include "assets/overlays/ovl_Bg_Numa_Hana/ovl_Bg_Numa_Hana.c"
...
s32 BgNumaHana_SpawnOpenFlowerCollisionChild(BgNumaHana* this, PlayState* play) {
    ...
    child = Actor_SpawnAsChild(&play->actorCtx, &this->dyna.actor, play, ACTOR_BG_NUMA_HANA, this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.y, this->dyna.actor.world.pos.z, this->dyna.actor.shape.rot.x, this->dyna.actor.shape.rot.y, this->dyna.actor.shape.rot.z, BG_NUMA_HANA_TYPE_OPEN_FLOWER_COLLISION);
...
void BgNumaHana_Init(Actor* thisx, PlayState* play) {
    ...
    if (type == BG_NUMA_HANA_TYPE_OPEN_FLOWER_COLLISION) {
        CollisionHeader_GetVirtual(&gWoodenFlowerOpenedFlowerCol, &header);
        this->dyna.bgId = DynaPoly_SetBgActor(play, &play->colCtx.dyna, &this->dyna.actor, header);
        BgNumaHana_SetupDoNothing(this);
        this->dyna.actor.draw = NULL;
    } else {
        CollisionHeader_GetVirtual(&gWoodenFlowerClosedFlowerCol, &header);
        this->dyna.bgId = DynaPoly_SetBgActor(play, &play->colCtx.dyna, &this->dyna.actor, header);
...
void BgNumaHana_ClosedIdle(BgNumaHana* this, PlayState* play) {
    if (this->fire.state != FIRE_STATE_NOT_LIT) {
        ...
        Flags_SetEventChkInf(EVENTCHKINF_PURIFIED_WOODFALL_TEMPLE);
...
void BgNumaHana_UnfoldInnerPetals(BgNumaHana* this, PlayState* play) {
    ...
    } else Actor_PlaySfx_Flagged(&this->dyna.actor, NA_SE_EV_FLOWERPETAL_MOVE - SFX_FLAG);
...
void BgNumaHana_RaiseFlower(BgNumaHana* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->dyna.actor.world.pos.y, this->dyna.actor.home.pos.y + 210.0f, this->dyna.actor.velocity.y)) {
        child = (DynaPolyActor*)this->dyna.actor.child;
...
void BgNumaHana_Draw(Actor* thisx, PlayState* play2) {
    ...
    OPEN_DISPS(play->state.gfxCtx, "../z_bg_numa_hana.c", 369);

    Gfx_SetupDL_25Opa(play->state.gfxCtx);
    MATRIX_FINALIZE_AND_LOAD(POLY_OPA_DISP++, play->state.gfxCtx, "../z_bg_numa_hana.c", 372);
    ...
    for (i = 0; i < ARRAY_COUNT(this->innerPetalPosRot); i++) {
        ...
        Matrix_SetTranslateRotateYXZ(innerPetalPosRot->pos.x, innerPetalPosRot->pos.y, innerPetalPosRot->pos.z, &innerPetalPosRot->rot);
        Matrix_Scale(0.1f, 0.1f, 0.1f, MTXMODE_APPLY);
        MATRIX_FINALIZE_AND_LOAD(POLY_OPA_DISP++, play->state.gfxCtx, "../z_bg_numa_hana.c", 381);
        gSPDisplayList(POLY_OPA_DISP++, gWoodenFlowerInnerPetalDL);

        Matrix_SetTranslateRotateYXZ(outerPetalPosRot->pos.x, outerPetalPosRot->pos.y, outerPetalPosRot->pos.z, &outerPetalPosRot->rot);
        Matrix_Scale(0.1f, 0.1f, 0.1f, MTXMODE_APPLY);
        MATRIX_FINALIZE_AND_LOAD(POLY_OPA_DISP++, play->state.gfxCtx, "../z_bg_numa_hana.c", 386);
    ...
    if ((objectSlot >= 0) && Object_IsLoaded(&play->objectCtx, objectSlot)) {
        Matrix_SetTranslateRotateYXZ(this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.y - 64.5f, this->dyna.actor.world.pos.z, &this->dyna.actor.shape.rot);
        Matrix_Scale(1.5f, 1.5f, 1.5f, MTXMODE_APPLY);
        MATRIX_FINALIZE_AND_LOAD(POLY_OPA_DISP++, play->state.gfxCtx, "../z_bg_numa_hana.c", 394);
    ...
    CLOSE_DISPS(play->state.gfxCtx, "../z_bg_numa_hana.c", 401);
}
