...
void BgKin2Shelf_Init(Actor* thisx, struct PlayState* play);
void BgKin2Shelf_Destroy(Actor* thisx, struct PlayState* play);
void BgKin2Shelf_Update(Actor* thisx, struct PlayState* play);
void BgKin2Shelf_Draw(Actor* thisx, struct PlayState* play);

void BgKin2Shelf_80B700A8(BgKin2Shelf* this);
void BgKin2Shelf_80B700C0(BgKin2Shelf* this, struct PlayState* play);
void BgKin2Shelf_80B70214(BgKin2Shelf* this);
void BgKin2Shelf_80B70230(BgKin2Shelf* this, struct PlayState* play);
void BgKin2Shelf_80B70498(BgKin2Shelf* this);
void BgKin2Shelf_80B704B4(BgKin2Shelf* this, struct PlayState* play);
...
s32 BgKin2Shelf_80B6FB30(BgKin2Shelf* this, PlayState* play) {
    s32 temp_v1 = BGKIN2SHELF_GET_1(&this->dyna.actor);
    f32 phi_f0, spA4;
    s16 spA2;
    Vec3f sp94, sp88, sp7C, sp70, sp64, sp58;
    CollisionPoly* sp54;
    s32 bgId;
    ...
    spA4 = D_80B70750[temp_v1] + ((this->dyna.unk_150 < 0.0f) ? 28.0f : -2.0f) + D_80B70768[temp_v1];

    Matrix_Translate(0.0f, 10.0f, D_80B70760[temp_v1], MTXMODE_NEW);
    Matrix_RotateY(BINANG_TO_RAD(spA2), MTXMODE_APPLY);
    ...
    return BgCheck_EntityLineTest3(&play->colCtx, &sp7C, &sp70, &sp64, &sp54, true, false, false, true, &bgId, &this->dyna.actor, 0.0f);
}

s32 BgKin2Shelf_80B6FCA4(BgKin2Shelf* this, PlayState* play) {
    Vec3f sp90, sp84, sp78, sp6C, sp60, sp54;
    CollisionPoly* sp50;
    f32 spA0;
    s16 sp9E;
    s32 bgId;
    ...
    spA0 = D_80B70758[0] + ((this->dyna.unk_150 < 0.0f) ? 28.0f : -2.0f) + D_80B70770[0];

    Matrix_Translate(0.0f, 10.0f, *D_80B70760, MTXMODE_NEW);
    Matrix_RotateY(BINANG_TO_RAD(sp9E), MTXMODE_APPLY);
    ...
    return BgCheck_EntityLineTest3(&play->colCtx, &sp78, &sp6C, &sp60, &sp50, true, false, false, true, &bgId,  &this->dyna.actor, 0.0f);
}

s32 BgKin2Shelf_80B6FE08(BgKin2Shelf* this, PlayState* play) {
    if (this->unk_164 & (0x8 | 0x2))
        return BgKin2Shelf_80B6FB30(this, play);
    return BgKin2Shelf_80B6FCA4(this, play);
}

s32 BgKin2Shelf_80B6FE48(BgKin2Shelf* this, PlayState* play) {
...
s32 BgKin2Shelf_80B6FEBC(BgKin2Shelf* this, PlayState* play) {
    if (this->unk_164 & 1)
...
bool BgKin2Shelf_80B6FF28(BgKin2Shelf* this, PlayState* play) {
    return (this->unk_165 > 8) && BgKin2Shelf_80B6FE48(this, play) && BgKin2Shelf_80B6FEBC(this, play) && !BgKin2Shelf_80B6FE08(this, play);
}
...
void BgKin2Shelf_80B700A8(BgKin2Shelf* this) {
    this->unk_165 = 0;
    this->actionFunc = BgKin2Shelf_80B700C0;
}

void BgKin2Shelf_80B700C0(BgKin2Shelf* this, PlayState* play) {
    s16 temp_v0;
    u8 phi_v0_2;

    if (fabsf(this->dyna.unk_150) > 0.1f) {
        temp_v0 = this->dyna.unk_158 - this->dyna.actor.shape.rot.y;
        ...
        if (BgKin2Shelf_80B6FF28(this, play)) {
            if (this->unk_164 & (0x8 | 0x2))
                BgKin2Shelf_80B70214(this);
            else BgKin2Shelf_80B70498(this);
        } else {
            GET_PLAYER(play)->stateFlags2 &= ~PLAYER_STATE2_4;
...
void BgKin2Shelf_80B70214(BgKin2Shelf* this) {
    this->unk_160 = 0.0f;
    this->actionFunc = BgKin2Shelf_80B70230;
}

void BgKin2Shelf_80B70230(BgKin2Shelf* this, PlayState* play) {
    f32 phi_f20, phi_f2;
    ...
    else phi_f2 = (Math_SinS(this->unk_160 * 0x8000) * 0.003f) + 0.009f;
    this->unk_160 += phi_f2;
    ...
    if (this->unk_160 >= 1.0f) {
        ...
        else phi_f20 = this->unk_166 + this->unk_160;
        this->dyna.actor.world.pos.x = ((Math_SinS(sp36) * phi_f20) * D_80B70750[sp40]) + this->dyna.actor.home.pos.x;
        this->dyna.actor.world.pos.z = ((Math_CosS(sp36) * phi_f20) * D_80B70750[sp40]) + this->dyna.actor.home.pos.z;
    }
}
...
void BgKin2Shelf_80B70498(BgKin2Shelf* this) {
    this->unk_160 = 0.0f;
    this->actionFunc = BgKin2Shelf_80B704B4;
}

void BgKin2Shelf_80B704B4(BgKin2Shelf* this, PlayState* play) {
    f32 temp_f20, phi_f20;
    s32 sp40 = BGKIN2SHELF_GET_1(&this->dyna.actor);
    ...
    if (this->unk_160 >= 1.0f) {
        ...
        else temp_f20 = this->unk_167 + this->unk_160;
        this->dyna.actor.world.pos.x = ((Math_SinS(sp36) * temp_f20) * D_80B70758[sp40]) + this->dyna.actor.home.pos.x;
        this->dyna.actor.world.pos.z = ((Math_CosS(sp36) * temp_f20) * D_80B70758[sp40]) + this->dyna.actor.home.pos.z;
    }
}
