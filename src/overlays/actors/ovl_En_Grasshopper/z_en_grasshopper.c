...
void EnGrasshopper_Init(Actor* thisx, struct PlayState* play);
void EnGrasshopper_Destroy(Actor* thisx, struct PlayState* play);
void EnGrasshopper_Update(Actor* thisx, struct PlayState* play);
void EnGrasshopper_Draw(Actor* thisx, struct PlayState* play);
...
void EnGrasshopper_DecideAction(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_SetupFly(EnGrasshopper* this);
void EnGrasshopper_Fly(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_RoamInCircles(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_SetupBank(EnGrasshopper* this);
void EnGrasshopper_Bank(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_SetupBounced(EnGrasshopper* this);
void EnGrasshopper_Bounced(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_SetupApproachPlayer(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_ApproachPlayer(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_SetupAttack(EnGrasshopper* this);
void EnGrasshopper_Attack(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_SetupWaitAfterAttack(EnGrasshopper* this);
void EnGrasshopper_WaitAfterAttack(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_Damaged(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_Dead(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_SetupFall(EnGrasshopper* this);
void EnGrasshopper_Fall(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_InitializeEffect(EnGrasshopper* this, Vec3f* pos);
void EnGrasshopper_UpdateEffects(EnGrasshopper* this, struct PlayState* play);
void EnGrasshopper_DrawEffects(EnGrasshopper* this, struct PlayState* play);
...
static DamageTable sDamageTable = {
    ...
    /* Deku stick    */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    /* Slingshot     */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Explosive     */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    /* Boomerang     */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Normal arrow  */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    /* Hammer swing  */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    /* Hookshot      */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_HOOK),
    ...
    /* Ice magic     */ DMG_ENTRY(3, DRAGONFLY_DMGEFF_NONE),
    /* Light magic   */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_NONE),
    /* Shield        */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_IMMUNE),
    /* Mirror Ray    */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_NONE),
    /* Kokiri spin   */ DMG_ENTRY(1, DRAGONFLY_DMGEFF_NONE),
    /* Giant spin    */ DMG_ENTRY(4, DRAGONFLY_DMGEFF_NONE),
    /* Master spin   */ DMG_ENTRY(2, DRAGONFLY_DMGEFF_NONE),
    ...
    /* Unknown 1     */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_NONE),
    /* Unblockable   */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_NONE),
    /* Hammer jump   */ DMG_ENTRY(4, DRAGONFLY_DMGEFF_NONE),
    /* Unknown 2     */ DMG_ENTRY(0, DRAGONFLY_DMGEFF_NONE),
};
...
void EnGrasshopper_Init(Actor* thisx, PlayState* play) {
    ...
    this->actor.naviEnemyId = NAVI_ENEMY_DRAGONFLY;
    ...
    this->collider.elements[0].dim.modelSphere.center.x = this->collider.elements[0].dim.modelSphere.center.y = this->collider.elements[0].dim.modelSphere.center.z = this->collider.elements[1].dim.modelSphere.center.y = this->collider.elements[1].dim.modelSphere.center.z = 0;
    ...
    if (this->type != DRAGONFLY_TYPE_GROWS_WHEN_SPAWNED)
        ...
        this->collider.elements[0].dim.modelSphere.radius = this->collider.elements[1].dim.modelSphere.radius = this->type;
    }
...
void EnGrasshopper_Destroy(Actor* thisx, PlayState* play) {
    ...
    //! @bug If the dragonfly selected a random index in EnGrasshopper_Init (because all indices were occupied), then two dragonflies will have the same index. When one of those dragonflies sharing an index is destroyed, it will mark the index as unoccupied, when it still occupied by at least one dragonfly.
...
void EnGrasshopper_ChangeAnim(EnGrasshopper* this, s32 animIndex) {
    ...
    morphFrames = 0.0f;
    if ((animIndex == DRAGONFLY_ANIM_ATTACK) || (animIndex == DRAGONFLY_ANIM_HOVER) || (animIndex == DRAGONFLY_ANIM_DAMAGE))
        morphFrames = -3.0f;
        ...
        if (this->decision == DRAGONFLY_DECISION_ATTACK)
            ...
            if (this->decision != DRAGONFLY_DECISION_ROAM_IN_CIRCLES) {
                if (Player_GetMask(play) == PLAYER_MASK_SKULL)
                    EnGrasshopper_SetupFly(this);
                else EnGrasshopper_SetupApproachPlayer(this, play);
            } else {
                this->timer = 0;
...
void EnGrasshopper_Fly(EnGrasshopper* this, PlayState* play) {
    ...
    if (Player_GetMask(play) != PLAYER_MASK_SKULL && !this->shouldTurn && (this->actor.xzDistToPlayer < 200.0f))
...
void EnGrasshopper_ApproachPlayer(EnGrasshopper* this, PlayState* play) {
    ...
    if (WaterBox_GetSurface1(play, &play->colCtx, this->tailTipPos.x, this->tailTipPos.z, &this->waterSurface, &waterBox)) {
    ...
    if ((this->timer == 0) || ((fabsf(this->targetApproachPos.x - this->actor.world.pos.x) <= 10.0f) && (fabsf(this->targetApproachPos.y - this->actor.world.pos.y) <= 20.0f) && (fabsf(this->targetApproachPos.z - this->actor.world.pos.z) <= 10.0f)))
...
void EnGrasshopper_Attack(EnGrasshopper* this, PlayState* play) {
    ...
    if ((this->collider.base.atFlags & AT_BOUNCED) || ((player->stateFlags1 & PLAYER_STATE1_SHIELDING) && (playerToHitPosDist <= 60.0f) && ((s16)((player->actor.shape.rot.y - this->actor.shape.rot.y) + 0x8000) < 0x2000) && ((s16)((player->actor.shape.rot.y - this->actor.shape.rot.y) + 0x8000) > -0x2000)))
        this->collider.elements[1].base.atElemFlags &= ~(ATELEM_ON | ATELEM_SFX_WOOD);
...
void EnGrasshopper_Dead(EnGrasshopper* this, PlayState* play) {
    ...
    Math_SmoothStepToS(&this->actor.world.rot.z, 0, 5, 0x3E8, 5);
    if (((this->drawDmgEffType == ACTOR_DRAW_DMGEFF_FROZEN_SFX) || (this->drawDmgEffType == ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX)) && (this->drawDmgEffTimer < 2)) {
        Actor_SpawnIceEffects(play, &this->actor, this->bodyPartsPos, DRAGONFLY_BODYPART_MAX, 2, 0.3f, 0.2f);
        this->drawDmgEffTimer = 0;
        this->drawDmgEffType = ACTOR_DRAW_DMGEFF_FIRE;
    }
...
void EnGrasshopper_Fall(EnGrasshopper* this, PlayState* play) {
    ...
    if (WaterBox_GetSurface1(play, &play->colCtx, this->actor.world.pos.x, this->actor.world.pos.z, &waterSurface, &waterBox)) {
    ...
    if ((BgCheck_SphVsFirstPoly(&play->colCtx, &this->tailTipPos, 40.0f)) && (((this->drawDmgEffType == ACTOR_DRAW_DMGEFF_FROZEN_SFX)) || (this->drawDmgEffType == ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX)) && (this->drawDmgEffTimer != 0)) {
        Actor_SpawnIceEffects(play, &this->actor, this->bodyPartsPos, DRAGONFLY_BODYPART_MAX, 2, 0.3f, 0.2f);
        this->drawDmgEffTimer = 0;
        this->drawDmgEffType = ACTOR_DRAW_DMGEFF_FIRE;
    }
    ...
    if (isUnderWater || (BgCheck_SphVsFirstPoly(&play->colCtx, &this->tailTipPos, 10.0f))) {
        Vec3f firePos;
        Vec3f sFireVelocityAndAccel[] = { { 1.0f, 0.0f, 0.5f },   { 1.0f, 0.0f, -0.5f },  { -1.0f, 0.0f, 0.5f }, { -1.0f, 0.0f, -0.5f }, { 0.5f, 0.0f, 1.0f },   { -0.5f, 0.0f, 1.0f }, { 0.5f, 0.0f, -1.0f },  { -0.5f, 0.0f, -1.0f }, { 0.0f, 0.0f, 0.0f }, };
        s32 i;
...
void EnGrasshopper_UpdateDamage(EnGrasshopper* this, PlayState* play) {
    ...
    if ((this->collider.elements[0].base.acElemFlags & ACELEM_HIT) || (this->collider.elements[1].base.acElemFlags & ACELEM_HIT)) {
        this->collider.base.acFlags &= ~AC_HIT;
        if (this->action != DRAGONFLY_ACTION_DAMAGED && this->action != DRAGONFLY_ACTION_DEAD && this->action != DRAGONFLY_ACTION_FALL) {
            if (this->actor.colChkInfo.damageReaction == DRAGONFLY_DMGEFF_NONE)
                attackDealsDamage = true;
            else if (this->actor.colChkInfo.damageReaction == DRAGONFLY_DMGEFF_FIRE) {
                this->drawDmgEffTimer = 40;
                this->drawDmgEffType = ACTOR_DRAW_DMGEFF_FIRE;
                attackDealsDamage = true;
            } else if (this->actor.colChkInfo.damageReaction == DRAGONFLY_DMGEFF_FREEZE) {
            ...
            } else if (this->actor.colChkInfo.damageReaction == DRAGONFLY_DMGEFF_LIGHT_ORB) {
                ...
                Actor_Spawn(&play->actorCtx, play, ACTOR_EN_CLEAR_TAG, this->actor.focus.pos.x, this->actor.focus.pos.y, this->actor.focus.pos.z, 0, 0, 0, CLEAR_TAG_PARAMS(CLEAR_TAG_LARGE_LIGHT_RAYS));
                attackDealsDamage = true;
            }
        }
    }
    ...
    if (attackDealsDamage) {
        ...
        else EnGrasshopper_SetupDead(this, play);
    }
}
...
void EnGrasshopper_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    EnGrasshopper* this = (EnGrasshopper*)thisx;
    ...
    // @bug: These do not check for valid limbs. Limb index 24 is DRAGONFLY_LIMB_MAX, so it (and any index larger than it) is not tied to an actual limb. While checking for DRAGONFLY_LIMB_ROOT twice is not a bug by itself, it causes another bug below.
    if ((limbIndex == DRAGONFLY_LIMB_ROOT) || (limbIndex == DRAGONFLY_LIMB_TAIL_SEGMENT_2) || (limbIndex == DRAGONFLY_LIMB_TAIL_TIP) || (limbIndex == DRAGONFLY_LIMB_BACK_LEFT_UPPER_LEG) || (limbIndex == DRAGONFLY_LIMB_BACK_RIGHT_UPPER_LEG) || (limbIndex == DRAGONFLY_LIMB_FRONT_LEFT_UPPER_LEG) || (limbIndex == DRAGONFLY_LIMB_FRONT_LEFT_FOOT) || (limbIndex == DRAGONFLY_LIMB_FRONT_RIGHT_UPPER_LEG) || (limbIndex == DRAGONFLY_LIMB_HEAD) || (limbIndex == 24) || (limbIndex == 25) || (limbIndex == DRAGONFLY_LIMB_ROOT)) {
        // @bug: This code only works properly if all 12 elements of bodyPartsPos are updated every frame, since otherwise bodyPartIndex will be non-zero at the end of a frame. Despite the fact that there are 12 checks in the above if-statement, only 9 of them are valid and non-duplicate, so only 9 elements of bodyPartsPos are updated on each frame. As a result, three elements in bodyPartsPos will either be (0, 0, 0) or be a value carried over from the previous frame.
...
void EnGrasshopper_Draw(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, NULL, EnGrasshopper_PostLimbDraw, &this->actor);
    if ((this->type != DRAGONFLY_TYPE_WOODFALL_TEMPLE_FINAL_ROOM) && (this->type != DRAGONFLY_TYPE_WOODFALL)) {
        Matrix_RotateX(0, MTXMODE_NEW);
        ...
        for (i = 0; i < 5; i++) {
            SubS_GenShadowTex(this->shadowBodyPartsPos, &this->actor.world.pos, shadowTex, i / 5.0f, DRAGONFLY_SHADOW_BODYPART_MAX, sShadowSizes, sParentShadowBodyParts);
            //! FAKE: Needed to fix some regs and stack https://decomp.me/scratch/4wJBW
    ...
    if (this->drawDmgEffTimer != 0) {
        ...
        Actor_DrawDamageEffects(play, &this->actor, this->bodyPartsPos, DRAGONFLY_BODYPART_MAX, this->drawDmgEffScale, this->drawDmgEffFrozenSteamScale, drawDmgEffAlpha, this->drawDmgEffType);
    }
...
static void* sLightningTextures[] = { gEffLightning1Tex, gEffLightning2Tex, gEffLightning3Tex, gEffLightning4Tex, gEffLightning5Tex, gEffLightning6Tex, gEffLightning7Tex, gEffLightning8Tex, };

void EnGrasshopper_DrawEffects(EnGrasshopper* this, PlayState* play) {
    EnGrasshopperEffect* effect;
    s16 i;
    MtxF mfResult, mfTrans, mfScale, mfRot, mfTransBillboard, mfTransBillboardRot;
    Mtx* mtx;

    OPEN_DISPS(play->state.gfxCtx, "../z_en_grasshopper.c", 1190);
    ...
    for (i=0; i<ARRAY_COUNT(this->effects); i++, effect++) {
        if (effect->isEnabled) {
            ...
            if (mtx != NULL) {
                ...
                gSPSegment(POLY_XLU_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(sLightningTextures[effect->lightningIndex]));
                gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 255, 255, 255, 255);
                gDPSetEnvColor(POLY_XLU_DISP++, 200, 255, 255, 255);
                gSPDisplayList(POLY_XLU_DISP++, SEGMENTED_TO_VIRTUAL(gEffLightningDL));
            }
        }
    }

    CLOSE_DISPS(play->state.gfxCtx, "../z_en_grasshopper.c", 1217);
}
