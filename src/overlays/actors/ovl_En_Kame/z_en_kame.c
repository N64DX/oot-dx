...
void EnKame_Destroy(Actor* thisx, struct PlayState* play);
void EnKame_Update(Actor* thisx, struct PlayState* play);
void EnKame_Draw(Actor* thisx, struct PlayState* play);

void EnKame_SetupIdle(EnKame* this);
void EnKame_Idle(EnKame* this, struct PlayState* play);
void EnKame_SetupWalk(EnKame* this);
void EnKame_Walk(EnKame* this, struct PlayState* play);
void EnKame_SetupRetreatIntoShell(EnKame* this);
void EnKame_RetreatIntoShell(EnKame* this, struct PlayState* play);
void EnKame_SetupPrepareToAttack(EnKame* this);
void EnKame_PrepareToAttack(EnKame* this, struct PlayState* play);
void EnKame_SetupAttack(EnKame* this);
void EnKame_Attack(EnKame* this, struct PlayState* play);
void EnKame_SetupEmergeFromShell(EnKame* this);
void EnKame_EmergeFromShell(EnKame* this, struct PlayState* play);
void EnKame_Flip(EnKame* this, struct PlayState* play);
void EnKame_SetupStruggle(EnKame* this);
void EnKame_Struggle(EnKame* this, struct PlayState* play);
void EnKame_SetupFlipUpright(EnKame* this);
void EnKame_FlipUpright(EnKame* this, struct PlayState* play);
void EnKame_Stunned(EnKame* this, struct PlayState* play);
void EnKame_Damaged(EnKame* this, struct PlayState* play);
void EnKame_SetupDead(EnKame* this, struct PlayState* play);
void EnKame_Dead(EnKame* this, struct PlayState* play);
void EnKame_SetupBurstIntoFlames(EnKame* this);
void EnKame_BurstIntoFlames(EnKame* this, struct PlayState* play);
void EnKame_DrawSpikedSnapper(Actor* thisx, struct PlayState* play);
...
static CollisionCheckInfoInit sColChkInfoInit = { 8, 15, 30, 80 };
...
static DamageTable sDamageTable = {
    /* Deku nut      */ DMG_ENTRY(1, EN_KAME_DMGEFF_STUN),
    /* Deku stick    */ DMG_ENTRY(2, EN_KAME_DMGEFF_NONE),
    /* Slingshot     */ DMG_ENTRY(1, EN_KAME_DMGEFF_NONE),
    /* Explosive     */ DMG_ENTRY(2, EN_KAME_DMGEFF_FLIP_OVER),
    /* Boomerang     */ DMG_ENTRY(1, EN_KAME_DMGEFF_NONE),
    /* Normal arrow  */ DMG_ENTRY(2, EN_KAME_DMGEFF_NONE),
    /* Hammer swing  */ DMG_ENTRY(2, EN_KAME_DMGEFF_FLIP_OVER),
    /* Hookshot      */ DMG_ENTRY(1, EN_KAME_DMGEFF_HOOKSHOT),
    /* Kokiri sword  */ DMG_ENTRY(1, EN_KAME_DMGEFF_NONE),
    /* Master sword  */ DMG_ENTRY(2, EN_KAME_DMGEFF_NONE),
    /* Giant's Knife */ DMG_ENTRY(4, EN_KAME_DMGEFF_NONE),
    /* Fire arrow    */ DMG_ENTRY(2, EN_KAME_DMGEFF_FIRE),
    /* Ice arrow     */ DMG_ENTRY(4, EN_KAME_DMGEFF_FREEZE),
    /* Light arrow   */ DMG_ENTRY(2, EN_KAME_DMGEFF_LIGHT_ORB),
    /* Unk arrow 1   */ DMG_ENTRY(4, EN_KAME_DMGEFF_FLIP_OVER),
    /* Unk arrow 2   */ DMG_ENTRY(2, EN_KAME_DMGEFF_NONE),
    /* Unk arrow 3   */ DMG_ENTRY(2, EN_KAME_DMGEFF_NONE),
    /* Fire magic    */ DMG_ENTRY(0, EN_KAME_DMGEFF_NONE),
    /* Ice magic     */ DMG_ENTRY(3, EN_KAME_DMGEFF_NONE),
    /* Light magic   */ DMG_ENTRY(0, EN_KAME_DMGEFF_NONE),
    /* Shield        */ DMG_ENTRY(0, EN_KAME_DMGEFF_NONE),
    /* Mirror Ray    */ DMG_ENTRY(0, EN_KAME_DMGEFF_NONE),
    /* Kokiri spin   */ DMG_ENTRY(1, EN_KAME_DMGEFF_NONE),
    /* Giant spin    */ DMG_ENTRY(4, EN_KAME_DMGEFF_NONE),
    /* Master spin   */ DMG_ENTRY(2, EN_KAME_DMGEFF_NONE),
    /* Kokiri jump   */ DMG_ENTRY(2, EN_KAME_DMGEFF_NONE),
    /* Giant jump    */ DMG_ENTRY(8, EN_KAME_DMGEFF_NONE),
    /* Master jump   */ DMG_ENTRY(4, EN_KAME_DMGEFF_NONE),
    /* Unknown 1     */ DMG_ENTRY(0, EN_KAME_DMGEFF_NONE),
    /* Unblockable   */ DMG_ENTRY(0, EN_KAME_DMGEFF_NONE),
    /* Hammer jump   */ DMG_ENTRY(4, EN_KAME_DMGEFF_FLIP_OVER),
    /* Unknown 2     */ DMG_ENTRY(0, EN_KAME_DMGEFF_NONE),
};

static void* sEyeTextures[] = { gSnapperEyeOpenTex, gSnapperEyeHalfTex, gSnapperEyeClosedTex, gSnapperEyeHalfTex };

static InitChainEntry sInitChain[] = {
    ICHAIN_S8(naviEnemyId, NAVI_ENEMY_SNAPPER, ICHAIN_CONTINUE),
...
void EnKame_Init(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_InitFlex(play, &this->snapperSkelAnime, &gSnapperSkel, &gSnapperIdleAnim, this->snapperJointTable, this->snapperMorphTable, SNAPPER_LIMB_MAX);
    SkelAnime_InitFlex(play, &this->spikedSnapperSkelAnime, &gSpikedSnapperSkel, &gSpikedSnapperIdleAnim, this->spikedSnapperJointTable, this->spikedSnapperMorphTable, SPIKED_SNAPPER_LIMB_MAX);
    ...
    if (!sTexturesDesegmented) {
        ...
        for (i=0; i<ARRAY_COUNT(sEyeTextures); i++)
            sEyeTextures[i] = SEGMENTED_TO_VIRTUAL(sEyeTextures[i]);
...
/**
 * Plays the idle animation, then starts walking once the animation is over. If the player gets close enough to the Snapper without the Stone Mask equipped, it will retreat into its shell to begin attacking them.
 */
void EnKame_Idle(EnKame* this, PlayState* play) {
    ...
    if (Player_GetMask(play) != PLAYER_MASK_SKULL && !(player->stateFlags1 & PLAYER_STATE1_23) && this->actor.xzDistToPlayer < 240.0f)
...
/**
 * Walk semi-randomly around the Snapper's home until the timer reaches 0, then transition back to being idle. If the player gets close enough to the Snapper without the Stone Mask equipped, it will retreat into its shell to begin attacking them.
 */
void EnKame_Walk(EnKame* this, PlayState* play) {
    Player* player = GET_PLAYER(play);

    if ((Player_GetMask(play) != PLAYER_MASK_SKULL) && !(player->stateFlags1 & PLAYER_STATE1_23) &&
...
/**
 * Plays the animation of the Snapper retreating its shell, and scales its limbs appropriately to make it appear like its head and legs are going into the shell. Once the animation is over, the
 * Snapper will either prepare for an attack (if the player is close enough and not wearing the Stone Mask), or it will emerge from its shell.
 */
void EnKame_RetreatIntoShell(EnKame* this, PlayState* play) {
    Player* player = GET_PLAYER(play);

    if (SkelAnime_Update(&this->snapperSkelAnime)) {
        if (Player_GetMask(play) != PLAYER_MASK_SKULL && !(player->stateFlags1 & PLAYER_STATE1_23) && ((this->timer == 0) || (this->actor.xzDistToPlayer < 120.0f)))
...
/**
 * Sets the Snapper's speed and z-rotation based on its angular velocity. When the Snapper is done with the first part of its attack, it will slowly "wind up" before moving back to its home; by setting the speed to be a multiple
 * of its angular velocity, the Snapper will smootly ramp up its speed during the "wind up". As for the z-rotation, setting it in this manner gives the Snapper a subtle, almost-imperceptible tilt as it spins.
 */
...
/**
 * If the Snapper is moving quickly enough over certain surface materials, this function will spawn dirt or snow-colored dust on even-numbered frames. Additionally, this function is also responsible for playing the Snapper's spinning sound effect if its angular velocity is large enough.
 */
void EnKame_ProcessAttackEffects(EnKame* this, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) && (this->actor.speed >= 3.0f)) {
        if ((play->gameplayFrames % 2) == 0) {
            SurfaceMaterial surfaceMaterial = SurfaceType_GetMaterial(&play->colCtx, this->actor.floorPoly, this->actor.floorBgId);
...
void EnKame_SetupPrepareToAttack(EnKame* this) {
    ...
    // This if-statement is checking which of the two is the current phase. If the Snapper is in the first phase of the attack, then it just finished retreating into its shell, and it hasn't switched to its "spiked" form yet. Thus, we check the draw function here to determine which phase the Snapper is in.
...
void EnKame_PrepareToAttack(EnKame* this, PlayState* play) {
    // In EnKame_SetupPrepareToAttack, the timer is set to 15 if it was the first phase of the attack, so this is yet another way of checking the current phase. Gradually scale the spikes up, and once they reach their full size, enable AT collisions so the Snapper can damage the player.
    if (this->timer == 15) {
        ...
        // If it's the first phase of the attack, rotate the Snapper to face the player and set the targetPos to a fixed position in that direction. This will make them charge towards the player once they start moving.
    ...
    // Calling EnKame_SetAttackSpeed will set the Snapper's speed based on its current angular velocity; since we are gradually scaling up the angular velocity, this would gradually scale up the speed as well. However, during the
    // first phase of the attack, the Snapper should wait in place until it reaches its maximum angular velocity. This check will prevent us from changing the Snapper's speed during the first phase until we're just about to attack.
...
/**
 * Moves the Snapper towards its target while spinning until it hits a wall or reaches the target. Once that happens,the Snapper will either reduce its angular velocity to 0x300 and prepare for the second phase of the attack (if the
 * Snapper is currently in the first phase), or it will reduce its angular velocity to 0 and emerge from its shell (if the Snapper is currently in the second phase).
 */
void EnKame_Attack(EnKame* this, PlayState* play) {
    ...
    if (this->targetAngularVelocityY == -1) {
        ...
        if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) || absYawToTarget > 0x3000 || Actor_WorldDistXZToPoint(&this->actor, &this->targetPos) < 50.0f) {
            if (this->targetPos.y < this->actor.home.pos.y)
                this->targetAngularVelocityY = 0x300;
            else this->targetAngularVelocityY = 0;
        }
        return;
    }
    ...
    if (Math_ScaledStepToS(&this->angularVelocityY, this->targetAngularVelocityY, (s32)(this->angularVelocityY * 0.09f) + 45)) {
        ...
        else EnKame_SetupPrepareToAttack(this);
    }
...
/**
 * Plays the animation of the Snapper emerging from its shell, and scales its limbs appropriately to make it appear like its head and legs are coming out of the shell. Once the animation is over, this will make the Snapper start walking again.
 */
...
void EnKame_Struggle(EnKame* this, PlayState* play) {
    ...
    else EnKame_SetupFlipUpright(this);
}
...
void EnKame_FlipUpright(EnKame* this, PlayState* play) {
    if (SkelAnime_Update(&this->snapperSkelAnime)) {
        ...
        // This is the part of the animation where the Snapper starts to flip itself upright. During this flip, this actor needs to manually control when ActorShadow_DrawCircle is called, so the shadowDraw pointer is set to NULL. See EnKame_PostLimbDraw and EnKame_Draw for more information.
...
void EnKame_SetupStunned(EnKame* this) {
    ...
    if (this->collider.elem.acHitElem->atDmgInfo.dmgFlags & (DMG_ARROW_NORMAL | DMG_ARROW_FIRE | DMG_ARROW_ICE | DMG_ARROW_LIGHT))
        this->actor.world.rot.y = this->collider.base.ac->shape.rot.y;
    else this->actor.world.rot.y = Actor_WorldYawTowardActor(this->collider.base.ac, &this->actor);

    this->actionFunc = EnKame_Stunned;
}

/**
 * Stops the Snapper's animation and locks it in place until the stun timer reaches zero. After that, the Snapper either dies (if the attack that stunned it reduced its health to 0) or goes back to struggling. If the Snapper dies in this state, it skips the "finishing blow" screen flash and game freeze.
 */
...
void EnKame_SetupDead(EnKame* this, PlayState* play) {
    ...
    // This allows callers of this function to skip the "finishing blow" screen flash and game freeze as well as the rotation that happens if the Snapper is killed by a projectile. Only EnKame_Stunned calls this
    // while passing NULL instead of play; in practice, this means that the only way to skip this code is to kill the Snapper with an Ice Arrow, since that's the only attack that both stuns it and deals damage.
    if (play != NULL) {
...
/**
 * Launches the Snapper into the air and spins it around wildly. Makes the Snapper burst into flames once it touches the ground.
 */
void EnKame_Dead(EnKame* this, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) && (this->actor.velocity.y < 0.0f)) {
        ...
        //! @bug unreachable code: EnKame_SetupDead sets this variable to 0, and no other code that runs while the Snapper is in this state will change it to anything else.
        if (this->timer == 1)
            this->actor.colorFilterTimer = 100;
        else if (this->actor.colorFilterTimer == 0)
            Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_GRAY, 255, COLORFILTER_BUFFLAG_OPA, 100);
...
/**
 * After waiting for the timer to reach 0, this function gradually shrinks the actor and spawns blue flames around it. Once the Snapper shrinks enough, this drops a random item and kills the actor.
 */
void EnKame_BurstIntoFlames(EnKame* this, PlayState* play) {
    ...
    if (this->timer > 0) {
        ...
        EffectSsDeadDb_Spawn(play, &flamePos, &gZeroVec3f, &gZeroVec3f, 100, 0, 255, 255, 255, 255, 0, 0, 255, 0, 9, false);
    }
}
...
void EnKame_UpdateDamage(EnKame* this, PlayState* play) {
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        Actor_SetDropFlag(&this->actor, &this->collider.elem, false);
        if ((this->drawDmgEffType == ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX) &&
        ...
        EnKame_Thaw(this, play);
        if (this->actor.colChkInfo.damageReaction == EN_KAME_DMGEFF_HOOKSHOT)
            return;

        if (this->actor.colChkInfo.damageReaction == EN_KAME_DMGEFF_INSTANT_KILL) {
            EnKame_SetupDead(this, play);
        } else if (this->actor.colChkInfo.damageReaction == EN_KAME_DMGEFF_FLIP_OVER) {
            if (this->collider.base.acFlags & AC_HARD)
                EnKame_SetupFlip(this);
            else if (!Actor_ApplyDamage(&this->actor))
                EnKame_SetupDead(this, play);
            else  EnKame_SetupDamaged(this);
        } else if (this->actionFunc == EnKame_Idle || this->actionFunc == EnKame_Walk) {
            EnKame_SetupRetreatIntoShell(this);
            this->timer = 21;
        } else if (!(this->collider.base.acFlags & AC_HARD)) {
            if (this->actor.colChkInfo.damageReaction == EN_KAME_DMGEFF_ELECTRIC_STUN) {
                this->stunTimer = 40;
            ...
            } else if (this->actor.colChkInfo.damageReaction == EN_KAME_DMGEFF_STUN) {
                this->stunTimer = 40;
                Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_BLUE, 255, COLORFILTER_BUFFLAG_OPA, 40);
                Actor_PlaySfx(&this->actor, NA_SE_EN_GOMA_JR_FREEZE);
                EnKame_SetupStunned(this);
            } else if (this->actor.colChkInfo.damageReaction == EN_KAME_DMGEFF_FREEZE) {
                EnKame_Freeze(this);
                ...
                if (this->actor.colChkInfo.damageReaction == EN_KAME_DMGEFF_FIRE) {
                    this->drawDmgEffScale = 0.6f;
                    this->drawDmgEffAlpha = 4.0f;
                    this->drawDmgEffType = ACTOR_DRAW_DMGEFF_FIRE;
                } else if (this->actor.colChkInfo.damageReaction == EN_KAME_DMGEFF_LIGHT_ORB) {
                    this->drawDmgEffScale = 0.6f;
                    this->drawDmgEffAlpha = 4.0f;
    ...
    if (play->actorCtx.unk_02 != 0 && this->actor.xyzDistToPlayerSq < SQ(200.0f) && (this->collider.base.acFlags & AC_ON)) {
        EnKame_Thaw(this, play);
        EnKame_SetupFlip(this);
    }
}
...
void EnKame_Update(Actor* thisx, PlayState* play) {
    ...
    Actor_UpdateBgCheckInfo(play, &this->actor, 40.0f, 60.0f, 40.0f, UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_1 | UPDBGCHECKINFO_FLAG_2 | UPDBGCHECKINFO_FLAG_3 | UPDBGCHECKINFO_FLAG_4);

    // When the Snapper is in the middle of the animation where it's flipping upright, setting the focus and updating the collider is handled in EnKame_Draw instead.
...
s32 EnKame_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnKame* this = (EnKame*)thisx;
    ...
    if ((this->actionFunc == EnKame_RetreatIntoShell) || (this->actionFunc == EnKame_EmergeFromShell)) {
        ...
        else if (limbIndex == SNAPPER_LIMB_BACK_RIGHT_LEG || limbIndex == SNAPPER_LIMB_BACK_LEFT_LEG || limbIndex == SNAPPER_LIMB_FRONT_RIGHT_LEG || limbIndex == SNAPPER_LIMB_FRONT_LEFT_LEG)
...
void EnKame_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    EnKame* this = (EnKame*)thisx;
    ...
    if (limbIndex == SNAPPER_LIMB_BODY) {
        ...
        // When the Snapper is flipping itself upright, the animation will make it appear as if it is jumping off the ground. This is purely visual, though, so if we do nothing, the actor's position won't actually change during this apparent "jump",
        // which causes certain things like the circle shadow and the actor's focus to look incorrect. To fix this, we temporarily adjust the actor's position during this animation so that everything looks correct. This position change is overridden
        // at the end of EnKame_Draw, so from the perspective of most other functions, nothing really changes.
        if (this->actor.shape.shadowDraw == NULL)
...
void EnKame_Draw(Actor* thisx, PlayState* play) {
    ...
    // If the Snapper is flipping itself upright, we'll update its position inside EnKame_PostLimbDraw to make it better match the "jump" that it does in the animation. We want to make sure that its original position is restored before we exit this function, however, so we save it here before we ever call EnKame_PostLimbDraw.
    if (this->actor.shape.shadowDraw == NULL)
        Math_Vec3f_Copy(&originalPos, &this->actor.world.pos);

    OPEN_DISPS(play->state.gfxCtx, "../z_en_kame.c", 904);

    Gfx_SetupDL_25Opa(play->state.gfxCtx);

    gSPSegment(POLY_OPA_DISP++, 0x08, sEyeTextures[this->eyeIndex]);

    SkelAnime_DrawFlexOpa(play, this->snapperSkelAnime.skeleton, this->snapperSkelAnime.jointTable, this->snapperSkelAnime.dListCount, EnKame_OverrideLimbDraw, EnKame_PostLimbDraw, &this->actor);
    Actor_DrawDamageEffects(play, &this->actor, this->bodyPartsPos, SNAPPER_BODYPART_MAX, this->drawDmgEffScale, this->drawDmgEffFrozenSteamScale, this->drawDmgEffAlpha, this->drawDmgEffType);

    // The Snapper is in the middle of the animation where it's flipping upright, and we updated its position in EnKame_PostLimbDraw as explained earlier. We draw the circle shadow, set the focus, and update its collider; all of these rely on the Snapper's position being updated earlier to match the "jump" that it
    // does in its animation. Once we're done, we restore the Snappper's position back to what it was before because we want other functions to act like the position never changed.
    if (this->actor.shape.shadowDraw == NULL) {
    ...
    CLOSE_DISPS(play->state.gfxCtx, "../z_en_kame.c", 922);
}

s32 EnKame_SpikedSnapperOverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnKame* this = (EnKame*)thisx;
...
void EnKame_DrawSpikedSnapper(Actor* thisx, PlayState* play) {
    ...
    Gfx_SetupDL_25Opa(play->state.gfxCtx);
    SkelAnime_DrawFlexOpa(play, this->spikedSnapperSkelAnime.skeleton, this->spikedSnapperSkelAnime.jointTable, this->spikedSnapperSkelAnime.dListCount, EnKame_SpikedSnapperOverrideLimbDraw, NULL, &this->actor);
}
