...
void ObjPurify_Update(Actor* thisx, PlayState* play) {
    ...
    if (OBJPURIFY_GET_UNK_FLAG(&this->dyna.actor) == 0 && player->actor.world.pos.y < this->dyna.actor.world.pos.y && Player_GetEnvironmentalHazard(play) >= PLAYER_ENV_HAZARD_SWIMMING && !(Flags_GetEventChkInf(EVENTCHKINF_PURIFIED_WOODFALL_TEMPLE)))
...
void func_80A851C8(Actor* thisx, PlayState* play) {
    ...
    OPEN_DISPS(play->state.gfxCtx, "../z_obj_purify.c", 223);

    if (opaDList != NULL) {
        Gfx_SetupDL_25Opa(play->state.gfxCtx);
        MATRIX_FINALIZE_AND_LOAD(POLY_OPA_DISP++, play->state.gfxCtx, "../z_obj_purify.c", 229);
        gSPDisplayList(POLY_OPA_DISP++, opaDList);
    }
    if (xluDList != NULL) {
        Gfx_SetupDL_25Xlu(play->state.gfxCtx);
        MATRIX_FINALIZE_AND_LOAD(POLY_XLU_DISP++, play->state.gfxCtx, "../z_obj_purify.c", 234);
        gSPDisplayList(POLY_XLU_DISP++, xluDList);
    }

    CLOSE_DISPS(play->state.gfxCtx, "../z_obj_purify.c", 238);
}
...
void func_80A85304(Actor* thisx, PlayState* play) {
    ...
    OPEN_DISPS(play->state.gfxCtx, "../z_obj_purify.c", 256);

    Gfx_SetupDL_25Xlu(play->state.gfxCtx);
    for (i = 0; i < ARRAY_COUNT(sp6C); i++) {
        index = sp6C[i];
        ObjPurify_SetSysMatrix(this->unk168[index]);
        MATRIX_FINALIZE_AND_LOAD(POLY_XLU_DISP++, play->state.gfxCtx, "../z_obj_purify.c", 263);
        gSPDisplayList(POLY_XLU_DISP++, info->xluDLists[index]);
    };

    CLOSE_DISPS(play->state.gfxCtx, "../z_obj_purify.c", 267);
}
