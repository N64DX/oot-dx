 ...
 * Note: This enemy code was mostly written by @trueffel but contains some AI code mostly for mathematical operations related to the pincer attack.
 */
...
#define FLAGS (ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_HOSTILE | ACTOR_FLAG_UPDATE_CULLING_DISABLED | ACTOR_FLAG_HOOKSHOT_PULLS_ACTOR | ACTOR_FLAG_TALK_WITH_C_UP) 

void EnSbeetle_Init(Actor* thisx, struct PlayState* play);
void EnSbeetle_Destroy(Actor* thisx, struct PlayState* play);
void EnSbeetle_Update(Actor* thisx, struct PlayState* play);
void EnSbeetle_Draw(Actor* thisx, struct PlayState* play);

s32 EnSbeetle_OverrideLimbDraw(struct PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx);
void EnSbeetle_PostLimbDraw(struct PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx);
...
void EnSbeetle_UpdatePincers(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_CheckHurt(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_UpdateBgCheck(EnSbeetle* this, struct PlayState* play);
bool EnSbeetle_HasLostPlayer(EnSbeetle* this, struct PlayState* play);
bool EnSbeetle_CheckPlayerNear(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SetupDoNothing(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_DoNothing(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_IdleActionWalk(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_IdleActionIdle2(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_IdleActionIdle3(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SetupThreatPlayer(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_ThreatPlayer(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SetupAttack(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_Attack(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SetupSwingAttack(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SwingAttack(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SetupHopAwayFromOrTowardsPlayer(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_HopAwayFromOrTowardsPlayer(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SetupStunned(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_Stunned(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SetupHurt(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_Hurt(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_SetupDie(EnSbeetle* this, struct PlayState* play);
void EnSbeetle_Die(EnSbeetle* this, struct PlayState* play);

ActorProfile En_Sbeetle_Profile = {
    /**/ ACTOR_EN_SBEETLE,  
    /**/ ACTORCAT_ENEMY,   
    /**/ FLAGS,
    /**/ OBJECT_SBEETLE, 
...
static ColliderCylinderInit sPincerLCylinderInit = { 
    ...
    {
        ELEM_MATERIAL_UNK0,
        { DMG_SLASH, HIT_SPECIAL_EFFECT_NONE, 0x30 },
...
static CollisionCheckInfoInit2 sColChkInit = { 6, 25.0f, 35.0f, 0, MASS_HEAVY };
...
static AnimationInfo sAnimationInfo[] = {
    ...
    { &gScissorsBeetleSkelAttackAnim, 1.5f, 0.0f, -1.0f, ANIMMODE_ONCE,        33.0f },
    { &gScissorsBeetleSkelSwingAnim,  1.0f, 0.0f, -1.0f, ANIMMODE_ONCE,        33.0f },
    { &gScissorsBeetleSkelHurtAnim,   1.5f, 0.0f, -1.0f, ANIMMODE_ONCE,        33.0f },
    { &gScissorsBeetleSkelDieAnim,    1.0f, 0.0f, -1.0f, ANIMMODE_ONCE,        33.0f },
};
...
void EnSbeetle_Init(Actor* thisx, PlayState* play) {
    ...
    static f32 sScaleFactors[] = { 1.0f, 0.9f, 0.8f, 0.7f, 0.6f };
    u8 scaleParam = thisx->params & 0xFF;

    ActorShape_Init(&this->actor.shape, 0.0f, ActorShadow_DrawCircle, 10.0f);
    Actor_SetScale(&this->actor, 0.1f * ((scaleParam <= 4) ? sScaleFactors[scaleParam] : 1.0f));
    ...
    SkelAnime_InitFlex(play, &this->skelAnime, &gScissorsBeetleSkel, &gScissorsBeetleSkelIdle1Anim, this->jointTable, this->morphTable, GSCISSORSBEETLESKEL_NUM_LIMBS);
...
void EnSbeetle_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->pincerState == ENSBEETLE_PINCER_OUTBOUND || this->pincerState == ENSBEETLE_PINCER_RETURN || this->pincerState == ENSBEETLE_PINCER_FAST_RETURN) { // Update pincer collider positions
...
void EnSbeetle_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->spawnIceTimer != 0) { // Spawn chunks of ice all over the Scissor Beetle's body
    ...
    SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->jointTable, this->skelAnime.dListCount, EnSbeetle_OverrideLimbDraw, EnSbeetle_PostLimbDraw, this);
}

/*
 * Converts a world-space position into the local coordinate space of the currently active matrix This is needed for bones as their position coordinates are bound to the actor
 */
void EnSbeetle_WorldToCurrentMatrixLocal(Vec3f* worldPos, Vec3f* localPos) {
    MtxF mtx;
    Vec3f delta;
    f32 scaleSqX, scaleSqY, scaleSqZ;
 ...
 */
void EnSbeetle_GetPincerPath(Vec3f* start, Vec3f* end, f32 progress, f32 side, Vec3f* result) {
    f32 dx, dz, length, perpendicularX, perpendicularZ, curve;
    s16 curveAngle;
...
/*
 *  If the scissors beetle takes damage while pincers are flying they fly back immediately stopping the attack
 */
void EnSbeetle_StartPincerFastReturn(EnSbeetle* this) {
...
void EnSbeetle_UpdatePincers(EnSbeetle* this, PlayState* play) {
    ...
    switch (this->pincerState) {
        case ENSBEETLE_PINCER_OUTBOUND:
            ...
            EnSbeetle_GetPincerPath(&this->pincerLHomePos, &this->pincerTargetPos, progress, -1.0f, &this->pincerLWorldPos);
            EnSbeetle_GetPincerPath(&this->pincerRHomePos, &this->pincerTargetPos, progress, 1.0f, &this->pincerRWorldPos);
        ...
        case ENSBEETLE_PINCER_RETURN:
            ...
            EnSbeetle_GetPincerPath(&this->pincerLReturnStart, &this->pincerLHomePos, progress, 1.0f, &this->pincerLWorldPos);
            EnSbeetle_GetPincerPath(&this->pincerRReturnStart, &this->pincerRHomePos, progress, -1.0f, &this->pincerRWorldPos);
        ...
        case ENSBEETLE_PINCER_FAST_RETURN:
            ...
            EnSbeetle_GetPincerPath(&this->pincerLReturnStart, &this->pincerLHomePos, progress, 0.2f, &this->pincerLWorldPos);
            EnSbeetle_GetPincerPath(&this->pincerRReturnStart, &this->pincerRHomePos, progress, -0.2f, &this->pincerRWorldPos);
...
s32 EnSbeetle_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    switch (limbIndex) {
        case GSCISSORSBEETLESKEL_PINCER_L_LIMB:
            ...
            if (this->pincerState == ENSBEETLE_PINCER_OUTBOUND || this->pincerState == ENSBEETLE_PINCER_RETURN || this->pincerState == ENSBEETLE_PINCER_FAST_RETURN) {
...
void EnSbeetle_CheckHurt(EnSbeetle* this, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_WATER) && this->actionFunc != EnSbeetle_Die) { // This enemy is not supposed to be in water so it dies immediately when in deep water
        EnSbeetle_SetupDie(this, play);
    }
}
...
/*
 * Checks whether the player has moved far enough away or changed elevation enough for the beetle to lose track of them.
 */
bool EnSbeetle_HasLostPlayer(EnSbeetle* this, PlayState* play) {
 ...
 */
bool EnSbeetle_CheckPlayerNear(EnSbeetle* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
    f32 yDist, xzDist;
    s16 yawToPlayer, yawDiff, absYawDiff;
...
void EnSbeetle_SetupDoNothing(EnSbeetle* this, PlayState* play) {
    this->actor.speed = 0.0f;
    this->nextIdleTimer = FRAMES(100);
    EnSbeetle_ChangeAnimation(this, SCISSORSBEETLE_ANIMATION_IDLE1);
    this->actionFunc = EnSbeetle_DoNothing;
}

/* 
 * Enemy has nothing to do. Constantly checking for the player. Random idle animations. Random walking
 */
        ...
        if (DECR(this->nextIdleTimer) == 0) {
            ...
            if (Rand_ZeroOne() > 0.4f) { // 50% chance of a random idle action
                ...
                if (randomIdle <= 0.3f) { // 30% chance of idle2
                    ...
                    this->randomWalkTimer = FRAMES(Rand_S16Offset(60, 40)); // Between 3 and 5 seconds
                    EnSbeetle_ChangeAnimation(this, SCISSORSBEETLE_ANIMATION_WALK);
                    this->idleAction = EnSbeetle_IdleActionWalk;
                }
            }
        }
    } else this->idleAction(this, play); // This is like a sub actionFunc
}
...
void EnSbeetle_IdleActionWalk(EnSbeetle* this, PlayState* play) {
    ...
    if (distToHome > 300.0f) // this way, the scissors beetle doesn't move off too much from the spawn position
        Math_ApproachS(&this->actor.world.rot.y, Math_Vec3f_Yaw(&this->actor.world.pos, &this->actor.home.pos), 3, 4000);
    else Math_ApproachS(&this->actor.world.rot.y, Rand_S16Offset(this->actor.world.rot.y, 0x400), 3, 4000);
    Math_ApproachS(&this->actor.shape.rot.y, this->actor.world.rot.y, 2, 6000);

    if (Animation_OnFrame(&this->skelAnime, 10.0f) || Animation_OnFrame(&this->skelAnime, 17.0f)) // foot touches the ground
...
void EnSbeetle_ThreatPlayer(EnSbeetle* this, PlayState* play) {
    ...
    Math_ApproachS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 3, 3000); // Rotate towards player
    ...
    if (DECR(this->attackTimer) == 0) {
        if (Rand_ZeroOne() < 0.6f) { // 50% chance
            ...
            else EnSbeetle_SetupSwingAttack(this, play); // 50% change for swinging the pincers
            return;
        }
        this->attackTimer = FRAMES(20);
    }
...
void EnSbeetle_SetupAttack(EnSbeetle* this, PlayState* play) {
    ...
    this->actor.speed = 0.0f;
    this->pincerLCollider.elem.atDmgInfo.damage = 0x30;
...
void EnSbeetle_Attack(EnSbeetle* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 24.0f) || Animation_OnFrame(&this->skelAnime, 25.0f)) { // Dash towards player
...
void EnSbeetle_SetupSwingAttack(EnSbeetle* this, PlayState* play) {
    ...
    this->actor.speed = 0.0f;
    this->pincerLCollider.elem.atDmgInfo.damage = 0x18;
...
void EnSbeetle_HopAwayFromOrTowardsPlayer(EnSbeetle* this, PlayState* play) {
    if (this->skelAnime.curFrame <= 6.0f || this->skelAnime.curFrame >= 15.0f) { // Rotate towards player before jumping
        ...
        else this->actor.speed = -12.0f; // Or away from the player
    }

    if (SkelAnime_Update(&this->skelAnime)) {
        if (EnSbeetle_HasLostPlayer(this, play))
            EnSbeetle_SetupDoNothing(this, play);
        else EnSbeetle_SetupThreatPlayer(this, play);
    }
}
...
void EnSbeetle_SetupDie(EnSbeetle* this, PlayState* play) {
    ...
    this->deathFreeze = 12;
    this->actor.speed = 0.0f;
    this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED; // Sbeetle not targetable anymore
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        if (DECR(this->deathFreeze) == 0) {
            ...
            if (this->actor.scale.x <= 0.001f) { // Enemy not visible anymore
                ...
                Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0xE0); // The Sbeetle might drop some random collectibles
