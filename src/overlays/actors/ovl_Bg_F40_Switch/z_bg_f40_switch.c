...
void BgF40Switch_Init(Actor* thisx, struct PlayState* play);
void BgF40Switch_Destroy(Actor* thisx, struct PlayState* play);
void BgF40Switch_Update(Actor* thisx, struct PlayState* play);
void BgF40Switch_Draw(Actor* thisx, struct PlayState* play);

void BgF40Switch_CheckAll(BgF40Switch* this, struct PlayState* play);
void BgF40Switch_Unpress(BgF40Switch* this, struct PlayState* play);
void BgF40Switch_IdlePressed(BgF40Switch* this, struct PlayState* play);
void BgF40Switch_Press(BgF40Switch* this, struct PlayState* play);
void BgF40Switch_WaitToPress(BgF40Switch* this, struct PlayState* play);
void BgF40Switch_IdleUnpressed(BgF40Switch* this, struct PlayState* play);
...
bool sBgF40SwitchGlobalsInitialized = false;
...
void BgF40Switch_CheckAll(BgF40Switch* this, PlayState* play) {
    if (play->gameplayFrames != sBgF40SwitchLastUpdateFrame) {
        ...
        s32 switchFlag, isPressed;
        u32 inCsMode = Player_InCsMode(play);

        for (actor = play->actorCtx.actorLists[ACTORCAT_SWITCH].head; actor != NULL; actor = actor->next) {
            if ((actor->id == ACTOR_BG_F40_SWITCH) && (actor->room == this->dyna.actor.room) &&
                ...
                if (actorAsSwitch->isPressed && (actorAsSwitch->actionFunc == BgF40Switch_IdlePressed)) { // Switch is fully pressed - if there's nothing keeping it pressed, wait a short time before reverting to unpressed state
                    ...
                    else actorAsSwitch->isPressed = isPressed;
                } else actorAsSwitch->isPressed = isPressed; // No delay when pressing switch, or releasing from a not-fully-pressed state
                if (actorAsSwitch->isPressed) {
                    ...
                    if (switchFlag >= 0 && switchFlag <= 0x3F) {
                        ...
                        if (!actorAsSwitch->wasPressed && (actorAsSwitch->actionFunc == BgF40Switch_IdleUnpressed) && !Flags_GetSwitch(play, switchFlag))
        ...
        for (actor = play->actorCtx.actorLists[ACTORCAT_SWITCH].head; actor != NULL; actor = actor->next) {
            if (actor->id == ACTOR_BG_F40_SWITCH && actor->room == this->dyna.actor.room && actor->update != NULL) {
                ...
                if (switchFlag >= 0 && switchFlag <= 0x3F && Flags_GetSwitch(play, switchFlag) && !(pressedSwitchFlags[(switchFlag & ~0x1F) >> 5] & (1 << (switchFlag & 0x1F))))
...
void BgF40Switch_Init(Actor* thisx, PlayState* play) {
    ...
    CollisionHeader_GetVirtual(&gStoneTowerFloorSwitchCol, &header);
...
void BgF40Switch_WaitToPress(BgF40Switch* this, PlayState* play) {
    if (!this->isInitiator && play->csCtx.state == CS_STATE_IDLE) {
