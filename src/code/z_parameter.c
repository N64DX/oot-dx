...
void Magic_DrawMeter(PlayState* play) {
    ...
    if (gSaveContext.save.info.playerData.magicLevel != 0) {
        ...
        OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gMagicMeterEndTex, 8, 16, R_MAGIC_METER_X * (HIRES ? 2 : 1), magicMeterY * (HIRES ? 2 : 1), 8 * (HIRES ? 2 : 1), 16 * (HIRES ? 2 : 1),
                                      (1 << 10) / (HIRES ? 2 : 1), (1 << 10) / (HIRES ? 2 : 1));

        OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gMagicMeterMidTex, 24, 16, (R_MAGIC_METER_X + 8) * (HIRES ? 2 : 1), magicMeterY * (HIRES ? 2 : 1),
                                      gSaveContext.magicCapacity * (HIRES ? 2 : 1), 16 * (HIRES ? 2 : 1), (1 << 10) / (HIRES ? 2 : 1), (1 << 10) / (HIRES ? 2 : 1));

        gDPLoadTextureBlock(OVERLAY_DISP++, gMagicMeterEndTex, G_IM_FMT_IA, G_IM_SIZ_8b, 8, 16, 0,
                            G_TX_MIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 3, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

        gSPTextureRectangle(OVERLAY_DISP++, ((R_MAGIC_METER_X + 8 + gSaveContext.magicCapacity) << 2) * (HIRES ? 2 : 1), (magicMeterY << 2) * (HIRES ? 2 : 1),
...
void Interface_Draw(PlayState* play) {
    ...
    if (pauseCtx->debugState == 0) {
        ...
        if (!(interfaceCtx->unk_1FA)) {
            ...
            gSPTextureRectangle(OVERLAY_DISP++, ((R_B_LABEL_X(gSaveContext.language) + B_LABEL_SHIFT) << 2) * (HIRES ? 2 : 1),
                                (R_B_LABEL_Y(gSaveContext.language) << 2) * (HIRES ? 2 : 1),
                                ((R_B_LABEL_X(gSaveContext.language) + B_LABEL_SHIFT + DO_ACTION_TEX_WIDTH) << 2) * (HIRES ? 2 : 1),
