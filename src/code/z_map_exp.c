...
void Minimap_DrawCompassIcons(PlayState* play) {
    ...
    if (play->interfaceCtx.minimapAlpha >= 0xAA) {
        ...
        mirrorOffset = ((mapWidth / 2) - ((R_COMPASS_OFFSET_X / 10) - (mapStartPosX - HIRES_DIVIDE(SCREEN_WIDTH) / 2 / SCALE_X))) * 2 * 10;
                ...
                if (!R_MINIMAP_DISABLED) {
                    ...
                    if (CHECK_DUNGEON_ITEM(DUNGEON_MAP, mapIndex)) {
                        ...
                        if (R_ENABLE_MIRROR == 1) {
                            ...
                            gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY((x) << 2) * SCALE_X, HIRES_MULTIPLY(R_DGN_MINIMAP_Y << 2), HIRES_MULTIPLY((x + MAP_I_TEX_WIDTH) << 2) * SCALE_X,
                                HIRES_MULTIPLY((R_DGN_MINIMAP_Y + MAP_I_TEX_HEIGHT) << 2), G_TX_RENDERTILE, (MAP_I_TEX_WIDTH - 1) << 5, 0, (-HIRES_DIVIDE(1 << 10)) / SCALE_X, HIRES_DIVIDE(1 << 10));
                        } else {
                            gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY((WS_SHIFT_FULL + R_DGN_MINIMAP_X) << 2) * SCALE_X, HIRES_MULTIPLY(R_DGN_MINIMAP_Y << 2), HIRES_MULTIPLY((WS_SHIFT_FULL + R_DGN_MINIMAP_X + MAP_I_TEX_WIDTH) << 2) * SCALE_X,
            ...
            case SCENE_ROAD_TO_FORTRESS:
                if (!R_MINIMAP_DISABLED) {
                    ...
                    if (R_ENABLE_MIRROR == 1) {
                        ...
                        gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY((x) << 2) * SCALE_X, HIRES_MULTIPLY(R_OW_MINIMAP_Y << 2), HIRES_MULTIPLY((x + gMapData->owMinimapWidth[mapIndex]) << 2) * SCALE_X, HIRES_MULTIPLY(((R_OW_MINIMAP_Y + gMapData->owMinimapHeight[mapIndex]) << 2)),
                            G_TX_RENDERTILE, (gMapData->owMinimapWidth[mapIndex] - 1) << 5, 0, (-HIRES_DIVIDE(1 << 10)) / SCALE_X, HIRES_DIVIDE(1 << 10));
                    } else {
                        gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY((WS_SHIFT_FULL + R_OW_MINIMAP_X) << 2) * SCALE_X, HIRES_MULTIPLY(R_OW_MINIMAP_Y << 2), HIRES_MULTIPLY((WS_SHIFT_FULL + R_OW_MINIMAP_X + gMapData->owMinimapWidth[mapIndex]) << 2) * SCALE_X, HIRES_MULTIPLY(((R_OW_MINIMAP_Y + gMapData->owMinimapHeight[mapIndex]) << 2)),
                    ...
                    if (((play->sceneId != SCENE_KAKARIKO_VILLAGE) && (play->sceneId != SCENE_KOKIRI_FOREST) &&
                        ...
                        if ((gMapData->owEntranceFlag[sEntranceIconMapIndex] != 0xFFFF) ||
                            ...
                            gSPTextureRectangle(OVERLAY_DISP++,
                                                HIRES_MULTIPLY((newX + WS_SHIFT_FULL) << 2) * SCALE_X,
                                                HIRES_MULTIPLY(gMapData->owEntranceIconPosY[sEntranceIconMapIndex] << 2),
                                                HIRES_MULTIPLY((newX + 8 + WS_SHIFT_FULL) << 2) * SCALE_X,
                    ...
                    if ((play->sceneId == SCENE_ZORAS_FOUNTAIN) &&
                        ...
                        gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY((WS_SHIFT_FULL + (R_ENABLE_MIRROR == 1 ? (257 + xOffset) : 270)) << 2) * SCALE_X, HIRES_MULTIPLY(154 << 2), HIRES_MULTIPLY((WS_SHIFT_FULL + (R_ENABLE_MIRROR ? (265 + xOffset) : 278)) << 2) * SCALE_X, HIRES_MULTIPLY(162 << 2), G_TX_RENDERTILE, 0,
