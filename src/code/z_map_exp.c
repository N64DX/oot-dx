...
void Minimap_DrawCompassIcons(PlayState* play) {
    ...
    if (play->interfaceCtx.minimapAlpha >= 0xAA) {
        ...
        Matrix_Translate(HIRES_MULTIPLY(((R_COMPASS_OFFSET_X + tempX + (WIDESCREEN ? 520 : 0)) / 10.0f)), HIRES_MULTIPLY(((R_COMPASS_OFFSET_Y - tempZ) / 10.0f)), 0.0f, MTXMODE_NEW);
        ...
        switch (play->sceneId) {
            ...
            case SCENE_ICE_CAVERN:
                if (!R_MINIMAP_DISABLED) {
                    ...
                    if (CHECK_DUNGEON_ITEM(DUNGEON_MAP, mapIndex)) {
                        ...
                        gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY((((WIDESCREEN ? 104 : 0) + R_DGN_MINIMAP_X) << 2)), HIRES_MULTIPLY((R_DGN_MINIMAP_Y << 2)),
                                            HIRES_MULTIPLY((((WIDESCREEN ? 104 : 0) + R_DGN_MINIMAP_X + MAP_I_TEX_WIDTH) << 2)),
            ...
            case SCENE_OUTSIDE_GANONS_CASTLE:
                if (!R_MINIMAP_DISABLED) {
                    ...
                    gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY((((WIDESCREEN ? 104 : 0) + R_OW_MINIMAP_X) << 2)), HIRES_MULTIPLY((R_OW_MINIMAP_Y << 2)),
                                        HIRES_MULTIPLY((((WIDESCREEN ? 104 : 0) + R_OW_MINIMAP_X + gMapData->owMinimapWidth[mapIndex]) << 2)),
                    ...
                    if (((play->sceneId != SCENE_KAKARIKO_VILLAGE) && (play->sceneId != SCENE_KOKIRI_FOREST) &&
                        ...
                        if ((gMapData->owEntranceFlag[sEntranceIconMapIndex] == 0xFFFF) ||
                            ...
                            gSPTextureRectangle(OVERLAY_DISP++,
                                                HIRES_MULTIPLY(((gMapData->owEntranceIconPosX[sEntranceIconMapIndex] + (WIDESCREEN ? 104 : 0)) << 2)),
                                                HIRES_MULTIPLY((gMapData->owEntranceIconPosY[sEntranceIconMapIndex] << 2)),
                                                HIRES_MULTIPLY(((gMapData->owEntranceIconPosX[sEntranceIconMapIndex] + 8 + (WIDESCREEN ? 104 : 0)) << 2)),
                    ...
                    if ((play->sceneId == SCENE_ZORAS_FOUNTAIN) &&
                        ...
                        gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY((((WIDESCREEN ? 104 : 0) + 270) << 2)), HIRES_MULTIPLY((154 << 2)), HIRES_MULTIPLY((((WIDESCREEN ? 104 : 0) + 278) << 2)), HIRES_MULTIPLY((162 << 2)), G_TX_RENDERTILE, 0,
                                            0, HIRES_DIVIDE((1 << 10)), HIRES_DIVIDE((1 << 10)));
                    }
