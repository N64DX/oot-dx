...
u8 Actor_AdjustSwordDamage(f32 damage, s32 dmgFlags, u8 itemAction) {
    if (dmgFlags & (DMG_SPIN_KOKIRI | DMG_SLASH_KOKIRI)) {
        if (IS_CHILD_QUEST_AS_CHILD && IS_HEROS_SWORD)
            damage = 1.5;
        else damage = 1;
...
void Actor_RestoreShieldDurability(u8 damage, s32 dmgFlags, u8 itemAction) {
    if (SHIELD_DURABILITY && damage > 0 && dmgFlags & (DMG_SLASH_KOKIRI | DMG_JUMP_KOKIRI | DMG_SPIN_KOKIRI) && itemAction == PLAYER_IA_SWORD_KOKIRI && IS_HEROS_SWORD) {
