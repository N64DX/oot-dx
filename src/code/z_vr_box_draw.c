...
void Skybox_Draw(SkyboxContext* skyboxCtx, GraphicsContext* gfxCtx, LightContext* lightCtx, s16 skyboxId, s16 blend, f32 x, f32 y, f32 z) {
    ...
    if (skyboxCtx->drawType != SKYBOX_DRAW_128) {
        ...
        if (skyboxId != SKYBOX_CUTSCENE_MAP) { // Render the bottom face black except in the cutscene map
            gDPPipeSync(POLY_OPA_DISP++);
