    ...
    while (true) {
        ...
        for (i = 0; i < mapMarkIconData->count; i++) {
            if ((mapMarkIconData->markType != MAP_MARK_CHEST) || !Flags_GetTreasure(play, markPoint->chestFlag)) {
                ...
                gSPTextureRectangle(OVERLAY_DISP++, HIRES_MULTIPLY(rectLeft) * SCALE_X, HIRES_MULTIPLY(rectTop), HIRES_MULTIPLY(markInfo->rectWidth + rectLeft) * SCALE_X,
                                    HIRES_MULTIPLY(rectTop + markInfo->rectHeight), G_TX_RENDERTILE, 0, 0, HIRES_DIVIDE(markInfo->dsdx) / SCALE_X,
