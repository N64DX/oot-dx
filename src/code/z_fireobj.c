...
void FireObj_StepSize(FireObj* fire) {
    ...
    if (fire->sizeGrowsCos2 == 1) {
        ...
        return;
    } else fire->yScale = fire->dynamicSize * fire->size;
    fire->xScale = fire->dynamicSize * fire->size;
}
...
void FireObj_UpdateStateTransitions(PlayState* play, FireObj* fire) {
    ...
    if (fire->ignitionDelay > 0) 
        ...
        else nextState = FIRE_STATE_SHRINKING;
        FireObj_SetState(fire, fire->dynamicSizeStep, nextState);
    }

    if ((fire->flags & FIRE_FLAG_WATER_EXTINGUISHABLE) && (fire->state != FIRE_STATE_NOT_LIT) && WaterBox_GetSurface1(play, &play->colCtx, fire->position.x, fire->position.z, &waterY, &waterBox) && ((waterY - fire->position.y) > (6500.0f * fire->yScale)))
        FireObj_SetState(fire, fire->dynamicSizeStep, FIRE_STATE_NOT_LIT);
    if ((fire->flags & FIRE_FLAG_INTERACT_STICK) && (player->heldItemAction == PLAYER_IA_DEKU_STICK)) {
        Math_Vec3f_Diff(MELEE_WEAPON_INFO_TIP(&player->meleeWeaponInfo[0]), &fire->position, &dist);
    ...
    if (sp40) {
        if (fire->state == FIRE_STATE_NOT_LIT) {
            ...
            player->unk_860 = 210;
            SfxSource_PlaySfxAtFixedWorldPos(play, &fire->position, 20, NA_SE_EV_FLAME_IGNITION);
...
void FireObj_Draw(PlayState* play, FireObj* fire) {
    ...
    if (fire->state != FIRE_STATE_NOT_LIT) {
        ...
        OPEN_DISPS(play->state.gfxCtx, "../z_fireobj", 134);

        Gfx_SetupDL_25Xlu(play->state.gfxCtx);
        gSPSegment(POLY_XLU_DISP++, 0x08, Gfx_TwoTexScroll(play->state.gfxCtx, 0, 0, 0, 32, 64, 1, 0, (fire->timer * -20) % 512U, 32, 128));

        gDPSetPrimColor(POLY_XLU_DISP++, 0, fireColors->lod, fireColors->primColor.r, fireColors->primColor.g, fireColors->primColor.b, fireColors->primColor.a);
        ...
        MATRIX_FINALIZE_AND_LOAD(POLY_XLU_DISP++, play->state.gfxCtx, "../z_fireobj", 149);
        gSPDisplayList(POLY_XLU_DISP++, gEffFire1DL);

        CLOSE_DISPS(play->state.gfxCtx, "../z_fireobj", 152);
    }
}
...
void FireObj_InitLight(PlayState* play, FireObjLight* light, u8* paramsIndex, Vec3f* pos) {
    ...
    Lights_PointGlowSetInfo(&light->lightInfo, pos->x, pos->y, pos->z, objectParams->color.r, objectParams->color.g, objectParams->color.b, objectParams->radius);
...
void FireObj_Update(PlayState* play, FireObj* fire, Actor* actor) {
    ...
    if (fire->state == FIRE_STATE_NOT_LIT) {
        if ((fire->collision.base.acFlags & AC_HIT) && (fire->collision.elem.acHitElem->atDmgInfo.dmgFlags & DMG_ARROW_FIRE))
            FireObj_SetState(fire, fire->dynamicSizeStep, FIRE_STATE_GROWING);
    } else if ((fire->collision.base.acFlags & AC_HIT) && arrow->actor.update != NULL && arrow->actor.id == ACTOR_EN_ARROW) {
