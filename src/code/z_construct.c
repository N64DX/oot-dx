...
void Interface_Init(PlayState* play) {
    ...
    if (IS_CHILD_QUEST) {
        ...
        if (INV_CONTENT(ITEM_BOW) == ITEM_BOW_FIRE || INV_CONTENT(ITEM_BOW) == ITEM_BOW_ICE || INV_CONTENT(ITEM_BOW) == ITEM_BOW_LIGHT) {
            INV_CONTENT(ITEM_BOW) = ITEM_BOW;
            for (i=0; i<3; i++)
                if (gSaveContext.save.info.equips.cButtonSlots[i] == SLOT_BOW)
                    gSaveContext.save.info.equips.buttonItems[i + 1] = ITEM_BOW;
        }
        if (INV_CONTENT(ITEM_ARROW_FIRE) >= ITEM_ROCS_FEATHER && INV_CONTENT(ITEM_ARROW_FIRE) <= ITEM_GOLDEN_FEATHER)
    ...
    for (i=0; i<8; i++) {
        ...
        if (item == ITEM_SWORDS)
            ...
            item = (TUNIC_EQUIP_TO_PLAYER(CUR_EQUIP_VALUE(EQUIP_TYPE_TUNIC)) == PLAYER_TUNIC_KOKIRI && IS_SPIRIT_TUNIC) ? ITEM_TUNIC_SPIRIT : ITEM_TUNIC_KOKIRI + TUNIC_EQUIP_TO_PLAYER(CUR_EQUIP_VALUE(EQUIP_TYPE_TUNIC));
...
void Regs_InitDataImpl(void) {
    ...
    R_B_LABEL_X(LANGUAGE_JPN) = B_BUTTON_X - 9;
    R_B_LABEL_X(LANGUAGE_ENG) = B_BUTTON_X - 12;
    ...
    R_B_LABEL_X(LANGUAGE_ENG) = B_BUTTON_X - 10;
    R_B_LABEL_X(LANGUAGE_GER) = B_BUTTON_X - 9;
    R_B_LABEL_X(LANGUAGE_FRA) = B_BUTTON_X - 11;
