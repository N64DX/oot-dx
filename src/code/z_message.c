...
#define DEFINE_MESSAGE_JPN(textId, type, yPos, jpnMessage, nesMessage, gerMessage, fraMessage) { textId, (_SHIFTL(type, 4, 4) | _SHIFTL(yPos, 0, 4)), _message_##textId##_jpn },
...
#define DEFINE_MESSAGE_NES(textId, type, yPos, jpnMessage, nesMessage, gerMessage, fraMessage) { textId, (_SHIFTL(type, 4, 4) | _SHIFTL(yPos, 0, 4)), _message_##textId##_nes },
...
void Message_DrawTextChar(PlayState* play, void* textureImage, Gfx** p) {
    ...
    if (msgCtx->textBoxType != TEXTBOX_TYPE_NONE_NO_SHADOW) {
        ...
        gSPTextureRectangle(gfx++, X_HIRES_MULTIPLY((x + R_TEXT_DROP_SHADOW_OFFSET + WS_SHIFT_HALF)) << 2, HIRES_MULTIPLY((y + R_TEXT_DROP_SHADOW_OFFSET) << 2),
                            X_HIRES_MULTIPLY((x + R_TEXT_DROP_SHADOW_OFFSET + WS_SHIFT_HALF + sCharTexSize)) << 2,
                            HIRES_MULTIPLY((y + R_TEXT_DROP_SHADOW_OFFSET + sCharTexSize) << 2), G_TX_RENDERTILE, 0, 0, X_HIRES_DIVIDE(sCharTexScale),
    ...
    gSPTextureRectangle(gfx++, X_HIRES_MULTIPLY((x + WS_SHIFT_HALF)) << 2, HIRES_MULTIPLY(y << 2), X_HIRES_MULTIPLY((x + WS_SHIFT_HALF + sCharTexSize)) << 2, HIRES_MULTIPLY((y + sCharTexSize) << 2), G_TX_RENDERTILE, 0, 0,
                        X_HIRES_DIVIDE(sCharTexScale), HIRES_DIVIDE(sCharTexScale));
    *p = gfx;
}
...
u8 Message_FindMessage(PlayState* play, u16 textId, Language language) {
    ...
    if (language == LANGUAGE_JPN || language == LANGUAGE_ENG) {
        messageTableEntry = (language == LANGUAGE_JPN) ? sJpnMessageEntryTablePtr : sNesMessageEntryTablePtr;
        seg               = messageTableEntry->segment;

        while (messageTableEntry->textId != 0xFFFF) {
            if (messageTableEntry->textId == textId) {
                foundSeg = messageTableEntry->segment;
                font->fontBuf[0] = messageTableEntry->typePos;
                messageTableEntry++;
                nextSeg = messageTableEntry->segment;
                font->msgOffset = foundSeg - seg;
                font->msgLength = nextSeg - foundSeg;
                return (font->msgLength > 8);
            }
            messageTableEntry++;
        }
    } else {
...
u16 Message_DrawItemIcon(PlayState* play, u16 itemId, Gfx** p, u16 i) {
    ...
    gSPTextureRectangle(gfx++, X_HIRES_MULTIPLY((msgCtx->textPosX + R_TEXTBOX_ICON_XPOS + WS_SHIFT_HALF)) << 2, HIRES_MULTIPLY(R_TEXTBOX_ICON_YPOS << 2),
                        X_HIRES_MULTIPLY((msgCtx->textPosX + R_TEXTBOX_ICON_XPOS + R_TEXTBOX_ICON_DIMENSION + WS_SHIFT_HALF)) << 2,
                        HIRES_MULTIPLY((R_TEXTBOX_ICON_YPOS + R_TEXTBOX_ICON_DIMENSION) << 2), G_TX_RENDERTILE, 0, 0, X_HIRES_DIVIDE(1 << 10), HIRES_DIVIDE(1 << 10));
    gDPPipeSync(gfx++);
    gDPSetCombineLERP(gfx++, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0);
...
void Message_DrawTextWide(PlayState* play, Gfx** gfxP) {
    ...
    if (msgCtx->msgMode >= MSGMODE_SCENE_TITLE_CARD_FADE_IN_BACKGROUND && msgCtx->msgMode <= MSGMODE_SCENE_TITLE_CARD_FADE_OUT_BACKGROUND && msgCtx->titleCardInfo != NULL) {
        msgCtx->textPosX = msgCtx->titleCardInfo->textPosX;
        msgCtx->textPosY = msgCtx->titleCardInfo->textPosY;
    } else {
            ...
            if (curCharWide == MESSAGE_WIDE_BOX_BREAK || curCharWide == MESSAGE_WIDE_TEXTID ||
                ...
                for (i = 0; i < playerNameLen; i++) {
                    ...
                    for (j = 0; j < FONT_CHAR_TEX_SIZE; j += 4) {
                        font->fontBuf[charTexIdx + j + 0] = fontBuf[j + 0];
                        font->fontBuf[charTexIdx + j + 1] = fontBuf[j + 1];
                        font->fontBuf[charTexIdx + j + 2] = fontBuf[j + 2];
                        font->fontBuf[charTexIdx + j + 3] = fontBuf[j + 3];
                    }
                    charTexIdx += FONT_CHAR_TEX_SIZE;
                }
...
#if !PLATFORM_IQUE
            else if (curChar == MESSAGE_UPGRADE_SIZE_PREFIX || curChar == MESSAGE_AGE_LAD || curChar == MESSAGE_AGE_ONE || curChar == MESSAGE_AGE_BOY || curChar == MESSAGE_AGE_GUY || curChar == MESSAGE_AGE_GUY_C || curChar == MESSAGE_PLURAL_TOKENS || curChar == MESSAGE_MASTER_SWORD) {
                const char* word = NULL;
                const char* const* words = NULL;
                u8 len, upgradeType, upgradeValue;

                switch (curChar) {
                    case MESSAGE_UPGRADE_SIZE_PREFIX:
                        upgradeType = MSG_BUF[++msgCtx->msgBufPos];
                        upgradeValue = CUR_UPG_VALUE(upgradeType) - 1;
                        if (!IS_CHILD_QUEST && upgradeValue == 2)
                            upgradeValue++;
                        words = upgrade_prefix_name_words[upgradeValue];
                        break;
...
void Message_OpenText(PlayState* play, u16 textId) {
    ...
    if (sTextIsCredits) {
        ...
        if (IS_CHILD_QUEST) {
            switch (textId) {
                case 0x0009: textId = 0x8013; break;
                case 0x000A: textId = 0x8014; break;
                case 0x0050: textId = 0x800D; break;
                case 0x0051: textId = 0x800E; break;
                case 0x0071: textId = 0x8012; break;
                case 0x00AA: textId = 0x8010; break;
                case 0x00AB: textId = 0x8011; break;
        ...
        if (!Message_FindMessage(play, textId, language) && language != LANGUAGE_ENG) {
            if (language == LANGUAGE_JPN) {
                DMA_REQUEST_SYNC(play->msgCtx.font.fontBuf, (uintptr_t)_nes_font_staticSegmentRomStart, _nes_font_staticSegmentRomEnd - _nes_font_staticSegmentRomStart, __FILE__, __LINE__);
        ...
        if (B_80121220 == NULL || B_80121220->unk_64 == NULL || !B_80121220->unk_64(&play->msgCtx.font))
            switch (language) {
                case LANGUAGE_ENG:
                     DMA_REQUEST_SYNC(MSG_BUF, (uintptr_t)_nes_message_data_staticSegmentRomStart + font->msgOffset, font->msgLength, __FILE__, __LINE__);
                     break;
                case LANGUAGE_GER:
                    DMA_REQUEST_SYNC(MSG_BUF, (uintptr_t)_ger_message_data_staticSegmentRomStart + font->msgOffset, font->msgLength, __FILE__, __LINE__);
                    break;
                case LANGUAGE_FRA:
                    DMA_REQUEST_SYNC(MSG_BUF, (uintptr_t)_fra_message_data_staticSegmentRomStart + font->msgOffset, font->msgLength, __FILE__, __LINE__);
                    break;
                case LANGUAGE_JPN:
                    DMA_REQUEST_SYNC(MSG_BUF, (uintptr_t)_jpn_message_data_staticSegmentRomStart + font->msgOffset, font->msgLength, __FILE__, __LINE__);
                    break;
                default:
                    break;
            }
    ...
    msgCtx->textBoxProperties = font->fontBuf[0];
...
void Message_PauseMenu_ShowDescription(PlayState* play, u16 textId, u8 textBoxPos) {
...
#if OOT_NTSC
    if (language == LANGUAGE_JPN && !setTempEnglish) {
        R_TEXT_CHAR_SCALE = 88;
        R_TEXT_LINE_SPACING = 18;
        R_TEXT_INIT_XPOS = 50;
    } else {
    ...
    if (IS_CHILD_QUEST_AS_CHILD) {
        if (textId == 0x0900 + ITEM_BOW)
            textId = 0x09FC;
        else if (textId == 0x0900 + ITEM_BROKEN_GORONS_SWORD)
            textId = 0x09FD;
        else if (textId == 0x0900 + ITEM_STRENGTH_SILVER_GAUNTLETS)
            textId = 0x09FE;
        else if (textId == 0x0900 + ITEM_STRENGTH_GOLD_GAUNTLETS)
            textId = 0x09FF;
        else if (textId == 0x0900 + ITEM_BOW_FIRE)
            textId = 0x0900 + ITEM_ARROW_FIRE;
        else if (textId == 0x0900 + ITEM_BOW_ICE)
            textId = 0x0900 + ITEM_ARROW_ICE;
        else if (textId == 0x0900 + ITEM_BOW_LIGHT)
            textId = 0x0900 + ITEM_ARROW_LIGHT;
    }
    ...
    textBoxType = msgCtx->textBoxType;
    if (textBoxType < TEXTBOX_TYPE_NONE_BOTTOM) {
        DMA_REQUEST_SYNC(msgCtx->textboxSegment, (uintptr_t)_message_staticSegmentRomStart, MESSAGE_STATIC_TEX_SIZE, __FILE__, __LINE__);
        msgCtx->textboxColorRed = 0;
        msgCtx->textboxColorGreen = 0;
        msgCtx->textboxColorBlue = 0;
        msgCtx->textboxColorAlphaTarget = 230;
...
void Message_DrawTextBox(PlayState* play, Gfx** p) {
    ...
    gSPTextureRectangle(gfx++, X_HIRES_MULTIPLY((R_TEXTBOX_X + WS_SHIFT_HALF)) << 2, HIRES_MULTIPLY(R_TEXTBOX_Y << 2), X_HIRES_MULTIPLY((R_TEXTBOX_X + WS_SHIFT_HALF + R_TEXTBOX_WIDTH)) << 2,
                        HIRES_MULTIPLY((R_TEXTBOX_Y + R_TEXTBOX_HEIGHT) << 2), G_TX_RENDERTILE, 0, 0, X_HIRES_DIVIDE(R_TEXTBOX_TEXWIDTH << 1),
    ...
    if (msgCtx->textBoxType == TEXTBOX_TYPE_OCARINA) {
        ...
        gDPLoadTextureBlock_4b(gfx++, gOcarinaTrebleClefTex, G_IM_FMT_I, 16, 32, 0, G_TX_MIRROR, G_TX_MIRROR,
                               G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
        gSPTextureRectangle(gfx++, X_HIRES_MULTIPLY((R_TEXTBOX_CLEF_XPOS + WS_SHIFT_HALF)) << 2, HIRES_MULTIPLY(R_TEXTBOX_CLEF_YPOS << 2), X_HIRES_MULTIPLY((R_TEXTBOX_CLEF_XPOS + WS_SHIFT_HALF + 16)) << 2,
                            HIRES_MULTIPLY((R_TEXTBOX_CLEF_YPOS + 32) << 2), G_TX_RENDERTILE, 0, 0, X_HIRES_DIVIDE(1 << 10), HIRES_DIVIDE(1 << 10));
    }
    ...
    if (msgCtx->msgLength != 0) {
        ...
        if (msgCtx->msgMode >= MSGMODE_OCARINA_PLAYING && msgCtx->msgMode < MSGMODE_TEXT_AWAIT_NEXT &&
            ...
            if (msgCtx->msgMode == MSGMODE_SONG_PLAYBACK) {
                ...
                for (notePosX = R_OCARINA_BUTTONS_XPOS + WS_SHIFT_HALF, i = 0; i < r; i++) {
                    gDPPipeSync(gfx++);
                    ...
                    gSPTextureRectangle(gfx++, X_HIRES_MULTIPLY(notePosX) << 2,
                                        HIRES_MULTIPLY(R_OCARINA_BUTTONS_YPOS(gOcarinaSongButtons[g].buttonsIndex[i]) << 2),
                                        X_HIRES_MULTIPLY((notePosX + 16)) << 2,
            ...
            if (msgCtx->msgMode != MSGMODE_SCARECROW_LONG_RECORDING_START &&
                msgCtx->msgMode != MSGMODE_MEMORY_GAME_START) {
                for (notePosX = R_OCARINA_BUTTONS_XPOS + WS_SHIFT_HALF, i = 0; i < 8; i++) {
                    ...
                    gSPTextureRectangle(gfx++, X_HIRES_MULTIPLY(notePosX) << 2, HIRES_MULTIPLY(R_OCARINA_BUTTONS_YPOS(sOcarinaButtonIndexBuf[i]) << 2),
                                        X_HIRES_MULTIPLY((notePosX + 16)) << 2,
                                        HIRES_MULTIPLY((R_OCARINA_BUTTONS_YPOS(sOcarinaButtonIndexBuf[i]) + 16) << 2), G_TX_RENDERTILE,
                                        0, 0, X_HIRES_DIVIDE(1 << 10), HIRES_DIVIDE(1 << 10));
                    notePosX += R_OCARINA_BUTTONS_XPOS_OFFSET;
                }
            }
        }
        ...
        switch (msgCtx->msgMode) {
            case MSGMODE_TEXT_START:
                ...
                if (var) {
                    ...
                    if (!msgCtx->textBoxPos) { // variable position
                        ...
                        R_TEXTBOX_Y_TARGET = sTextboxLowerYPositions[var];
                        if (play->pauseCtx.itemDescriptionOn)
                            R_TEXTBOX_Y_TARGET -= 12;
                    }
            ...
            case MSGMODE_TEXT_CLOSING:
                ...
                if (play->pauseCtx.itemDescriptionOn) {
                    ...
                    play->pauseCtx.itemDescriptionOn = false;
                }
...
#if PLATFORM_N64 && OOT_NTSC_N64
    if ((B_80121220 != NULL) && (B_80121220->unk_58 != NULL))
        B_80121220->unk_58(&sJpnMessageEntryTablePtr, &sNesMessageEntryTablePtr, &sGerMessageEntryTablePtr, &sFraMessageEntryTablePtr, &sStaffMessageEntryTablePtr);
#elif PLATFORM_N64 && OOT_NTSC
    if ((B_80121220 != NULL) && (B_80121220->unk_58 != NULL)) {
