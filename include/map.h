...
typedef struct MapData {
    /* 0x00 */ u8 (*floorTexIndexOffset)[8]; // dungeon texture index offset by floor
    /* 0x04 */ u8* bossFloor; // floor the boss is on
    /* 0x08 */ u8 (*roomPalette)[32]; // map palette by room
    /* 0x0C */ u8* maxPaletteCount; // max number of palettes in a same floor
    /* 0x10 */ u8 (*paletteRoom)[8][14]; // room by palette by floor
    ...
    /* 0x38 */ u8* dgnTexIndexBase; // dungeon texture index base
    /* 0x3C */ s16 (*dgnCompassInfo)[4]; // [X scale, Y scale, X offset, Y offset]
    /* 0x40 */ u8* owMinimapWidth;
    /* 0x44 */ u8* owMinimapHeight;
    ...
    /* 0x58 */ u8* switchEntryCount; // number of "room switch" entries, which correspond to the next 3 arrays
    ...
    /* 0x68 */ u8 (*floorID)[8];
    /* 0x6C */ s8* skullFloorIconY; // dungeon big skull icon Y pos
    /* 0x70 */ s8 (*dungeonXOffset)[44]; // dungeon minimap x-offset for mirror mode
    /* 0x74 */ s8* overworldXOffset; // overworld minimap x-offset for mirror mode
} MapData; // size = 0x78
