#ifndef MAP_SELECT_STATE_H
#define MAP_SELECT_STATE_H

#include "ultra64.h"
#include "game.h"
#include "view.h"

struct MapSelectState;

typedef struct MapSelectEntry {
    /* 0x00 */ char* name;
    /* 0x04 */ void (*loadFunc)(struct MapSelectState*, s32);
    /* 0x08 */ s32 entranceIndex;
} MapSelectEntry; // size = 0xC

typedef struct SaveSelectEntry {
    /* 0x00 */ u8 number;
    /* 0x04 */ char* name;
    /* 0x08 */ u8 type;
    /* 0x09 */ u16 flag;
} SaveSelectEntry; // size = 0x11

typedef enum FlagType {
    /* F7 */ EVENT = 0xF7,
    /* F8 */ ITEM,
    /* F9 */ INFTABLE,
    /* FA */ SCENE_CLEAR,
    /* FB */ CARPENTERS,
    /* FC */ NABOORU,
    /* FD */ MASK,
    /* FE */ WELL,
    /* FF */ SHADOW
} FlagType;

typedef struct MapSelectState {
    /* 0x0000 */ GameState state;
    /* 0x00A8 */ View view;
    /* 0x01D0 */ s16 count;
    /* 0x01D4 */ MapSelectEntry* entries;
    /* 0x01D4 */ SaveSelectEntry* saveEntries;
    /* 0x01D8 */ s16 currentEntry;
    /* 0x01D8 */ s16 currentEntryBackup;
    /* 0x01DC */ s16 pageDownIndex; // Index of pageDownStops
    /* 0x01DC */ s16 pageDownIndexBackup;
    /* 0x01E0 */ s16 pageDownStops[7];
    /* 0x0208 */ s8 roomNum;
    /* 0x0208 */ s8 questMode;
    /* 0x0208 */ s8 mirrorMode;
    /* 0x020C */ s16 topDisplayedEntry; // The entry which is currently at the top of the screen
    /* 0x020C */ s16 topDisplayedEntryBackup;
    /* 0x021C */ s32 verticalInputAccumulator;
    /* 0x0220 */ s32 verticalInput;
    /* 0x0224 */ s32 timerUp;
    /* 0x0228 */ s32 timerDown;
    /* 0x022C */ s32 lockUp;
    /* 0x0230 */ s32 lockDown;
    /* 0x0234 */ bool mainTab;
    /* 0x0238 */ u8* staticSegment;
} MapSelectState; // size = 0x240

void MapSelect_Init(GameState* thisx);
void MapSelect_Destroy(GameState* thisx);

#endif
